...
class Tournament {
  constructor(room, format, generator, playerCap, isRated) {
    ...
    if (
      ...
      this.playerCap > Config.tourdefaultplayercap
    ) {
      Monitor.log(
        `[TourMonitor] Room ${room.id} starting a tour over default cap (${this.playerCap})`
      );
    }
    ...
    room.add(
      `|tournament|create|${this.format}|${generator.name}|${this.playerCap}`
    );
    const update = {
      format: this.format,
  ...
  setGenerator(generator, output) {
    ...
    this.generator = generator;
    this.room.send(
      `|tournament|update|${JSON.stringify({ generator: generator.name })}`
    );
    this.isBracketInvalidated = true;
    this.update();
    return true;
  ...
  setCustomRules(rules, output) {
    try {
      this.teambuilderFormat = Dex.validateFormat(
        `${this.originalFormat}@@@${rules}`
      );
    } catch (e) {
  ...
  getCustomRules() {
    ...
    const html = [];
    if (bans.length)
      html.push(`<b>Bans</b> - ${Chat.escapeHTML(bans.join(", "))}`);
    if (unbans.length)
      html.push(`<b>Unbans</b> - ${Chat.escapeHTML(unbans.join(", "))}`);
    if (addedRules.length)
      html.push(
        `<b>Added rules</b> - ${Chat.escapeHTML(addedRules.join(", "))}`
      );
    if (removedRules.length)
      html.push(
        `<b>Removed rules</b> - ${Chat.escapeHTML(removedRules.join(", "))}`
      );
    return html.join(`<br />`);
  }
  ...
  updateFor(targetUser, connection) {
    ...
    if (this.format !== this.originalFormat)
      update.teambuilderFormat = this.originalFormat;
    connection.sendTo(
      this.room,
      `|tournament|update|${JSON.stringify(update)}`
    );
    if (this.isTournamentStarted && isJoined) {
      const update2 = {
        challenges: usersToNames(
          this.availableMatchesCache.challenges.get(
            this.players[targetUser.userid]
          )
        ),
        challengeBys: usersToNames(
          this.availableMatchesCache.challengeBys.get(
            this.players[targetUser.userid]
          )
        )
      };
      connection.sendTo(
        this.room,
        `|tournament|update|${JSON.stringify(update2)}`
      );

      const pendingChallenge = this.pendingChallenges.get(
        this.players[targetUser.userid]
      );
      if (pendingChallenge) {
  ...
  update(targetUser) {
    if (targetUser)
      throw new Error(
        "Please use updateFor() to update the tournament for a specific user."
      );
    if (this.isEnded) return;
    if (this.isBracketInvalidated) {
      if (
        Date.now() <
        this.lastBracketUpdate + BRACKET_MINIMUM_UPDATE_INTERVAL
      ) {
  ...
  addUser(user, isAllowAlts, output) {
    ...
    if (gameCount > 4) {
      output.errorReply(
        "Due to high load, you are limited to 4 games at the same time."
      );
      return;
    }
    ...
    if (data.type === "tree") {
      ...
      while (queue.length > 0) {
        ...
        if (node.state === "available") {
          let pendingChallenge = this.pendingChallenges.get(
            node.children[0].team
          );
          if (
            pendingChallenge &&
            node.children[1].team === pendingChallenge.to
          ) {
            node.state = "challenging";
          }

          let inProgressMatch = this.inProgressMatches.get(
            node.children[0].team
          );
        ...
        data.tableContents.forEach((row, r) => {
          ...
          if (pendingChallenge || inProgressMatch) {
            row.forEach((cell, c) => {
              ...
              if (
                pendingChallenge &&
                data.tableHeaders.cols[c] === pendingChallenge.to
              ) {
              ...
              if (
                inProgressMatch &&
                data.tableHeaders.cols[c] === inProgressMatch.to
              ) {
  ...
  getAvailableMatches() {
    ...
    if (typeof matches === "string") {
      this.room.add(
        `Unexpected error from getAvailableMatches(): ${matches}. Please report this to an admin.`
      );
      return;
    }
  ...
  disqualifyUser(userid, output, reason) {
    ...
    if (user) {
      ...
      if (reason !== null)
        user.popup(
          `|modal|You have been disqualified from the tournament in ${this.room
            .title + (reason ? ":\n\n" + reason : ".")}`
        );
    }
    this.isBracketInvalidated = true;
  ...
  runAutoDisqualify(output) {
    ...
    this.lastActionTimes.forEach((time, player) => {
      ...
      if (
        now > time + this.autoDisqualifyTimeout &&
        this.autoDisqualifyWarnings.has(player)
      ) {
        let reason;
        ...
        now >
        time + this.autoDisqualifyTimeout - AUTO_DISQUALIFY_WARNING_TIMEOUT
      ) {
  ...
  async challenge(user, targetUserid, output) {
    ...
    if (this.generator.getUserBusy(from) || this.generator.getUserBusy(to)) {
      this.room.add(
        "Tournament backend breaks specifications. Please report this to an admin."
      );
      return;
    }
    ...
    const ready = await Ladders(this.teambuilderFormat).prepBattle(
      output.connection
    );
    if (!ready) {
    ...
    from.sendRoom(
      `|tournament|update|${JSON.stringify({ challenging: to.name })}`
    );
    to.sendRoom(
      `|tournament|update|${JSON.stringify({ challenged: from.name })}`
    );
  ...
  async acceptChallenge(user, output) {
    ...
    this.room
      .add(`|tournament|battlestart|${from.name}|${user.name}|${room.id}`)
      .update();
  ...
  onBattleWin(room, winnerid) {
    ...
    if (result === "draw" && !this.generator.isDrawingSupported) {
      this.room.add(
        `|tournament|battleend|${from.name}|${to.name}|${result}|${score.join(
          ","
        )}|fail|${room.id}`
      );
    ...
    if (!(this.disqualifiedUsers.get(from) || this.disqualifiedUsers.get(to))) {
      ...
      if (error) {
        ...
        return this.room
          .add(
            `Unexpected ${error} from setMatchResult([${room.p1.userid}, ${room.p2.userid}], ${result}, ${score}) in onBattleWin(${room.id}, ${winnerid}). Please report this to an admin.`
          )
          .update();
      }
    }
  ...
  onTournamentEnd() {
    ...
    if (tourSize >= sizeRequiredToEarn && this.room.isOfficial) {
      ...
      if (
        Db.userBadges.has(wid) &&
        Db.userBadges.get(wid).indexOf("Tournament Champion") > -1
      )
        firstMoney = Math.ceil(firstMoney * 1.5);
      ...
      if (runnerUp) {
        ...
        if (
          Db.userBadges.has(rid) &&
          Db.userBadges.get(rid).indexOf("Tournament Champion") > -1
        )
          secondMoney = Math.ceil(firstMoney * 1.5);
      }
        ...
        Economy.readMoney(wid, newAmount => {
          ...
          Economy.logTransaction(
            Chat.escapeHTML(winner) +
              ...
              firstMoney +
              " " +
              (firstMoney === 1 ? global.currencyName : global.currencyPlural) +
              " from a tournament."
          );
        });
...
function createTournament(
  ...
  if (Rooms.global.lockdown) {
    output.errorReply(
      "The server is restarting soon, so a tournament cannot be created."
    );
    return;
  }
  ...
  if (format.effectType !== "Format" || !format.tournamentShow) {
    ...
    const formats = Object.values(Dex.formats)
      .filter(f => f.tournamentShow)
  ...
  room.game = exports.tournaments[room.id] = new Tournament(
    room,
    format,
    createTournamentGenerator(generator, args, output),
    playerCap,
    isRated
  );
  return room.game;
...
const commands = {
  basic: {
      ...
      if (tournament.isTournamentStarted) {
        if (
          tournament.generator
            .getUsers(true)
            .some(player => player.userid === user.userid)
        ) {
          tournament.disqualifyUser(user.userid, this);
        } else {
          this.errorReply(
            "You have already been eliminated from this tournament."
          );
        }
      } else {
    ...
    getusers: function(tournament) {
      ...
      this.sendReplyBox(
        `<strong>${
          users.length
        } users remain in this tournament:</strong><br />${Chat.escapeHTML(
          users.join(", ")
        )}`
      );
    },
    ...
    vtm: function(tournament, user, params, cmd, connection) {
      ...
      TeamValidatorAsync(tournament.teambuilderFormat)
    ...
    viewcustomrules: function(tournament) {
      ...
      if (tournament.customRules.length < 1) {
        return this.errorReply(
          "The tournament does not have any custom rules."
        );
      }
      this.sendReply(
  ...
  creation: {
    settype: function(tournament, user, params, cmd) {
      if (params.length < 1) {
        return this.sendReply(
          `Usage: ${cmd} <type> [, <comma-separated arguments>]`
        );
      }
      ...
      if (generator && tournament.setGenerator(generator, this)) {
        if (playerCap && playerCap >= 2) {
          ...
          if (
            Config.tourdefaultplayercap &&
            tournament.playerCap > Config.tourdefaultplayercap
          ) {
            Monitor.log(
              `[TourMonitor] Room ${tournament.room.id} starting a tour over default cap (${tournament.playerCap})`
            );
          }
        ...
        const capNote = tournament.playerCap
          ? " with a player cap of " + tournament.playerCap
          : "";
        this.privateModAction(
          `(${user.name} set tournament type to ${generator.name + capNote}.)`
        );
    ...
    setplayercap: function(tournament, user, params, cmd) {
      if (params.length < 1) {
        if (tournament.playerCap) {
          return this.sendReply(
            `Usage: ${cmd} <cap>; The current player cap is ${tournament.playerCap}`
          );
        } else {
      ...
      if (tournament.isTournamentStarted) {
        return this.errorReply(
          "The player cap cannot be changed once the tournament has started."
        );
      }
      ...
      if (playerCap === 0) {
        tournament.playerCap = 0;
        this.privateModAction(
          `(${user.name} removed the tournament's player cap.)`
        );
        ...
        if (isNaN(playerCap) || playerCap < 2)
          return this.errorReply(
            "The tournament cannot have a player cap less than 2."
          );
        if (playerCap === tournament.playerCap)
          return this.errorReply(
            `The tournament's player cap is already ${playerCap}.`
          );
        tournament.playerCap = playerCap;
        if (
          Config.tourdefaultplayercap &&
      ...
      this.room.send(
        `|tournament|update|{"playerCap": "${tournament.playerCap}"}`
      );
    },
    end: "delete",
    stop: "delete",
    ...
    customrules: function(tournament, user, params, cmd) {
      if (cmd === "banlist") {
        return this.errorReply(
          "The new syntax is: /tour rules -bannedthing, +unbannedthing, !removedrule, addedrule"
        );
      }
      if (params.length < 1) {
        this.sendReply("Usage: /tour rules <list of rules>");
        this.sendReply(
          "Rules can be: -bannedthing, +unbannedthing, !removedrule, addedrule"
        );
      ...
      if (tournament.setCustomRules(params, this)) {
        ...
        this.privateModAction(
          `(${user.name} updated the tournament's custom rules.)`
        );
    ...
    clearcustomrules: function(tournament, user) {
      ...
      this.privateModAction(
        `(${user.name} cleared the tournament's custom rules.)`
      );
    ...
    setname: function(tournament, user, params, cmd) {
      ...
      if (name.includes("|"))
        return this.errorReply(
          "The tournament's name cannot include the | symbol."
        );
      tournament.format = name;
      this.room.send(
        `|tournament|update|${JSON.stringify({ format: tournament.format })}`
      );
      this.privateModAction(
        `(${user.name} set the tournament's name to ${tournament.format}.)`
      );
    ...
    clearname: function(tournament, user) {
      if (tournament.format === tournament.originalFormat)
  ...
  moderation: {
    ...
    disqualify: function(tournament, user, params, cmd) {
      ...
      if (tournament.disqualifyUser(targetUserid, this, reason)) {
        this.privateModAction(
          `(${targetUser.name ||
            targetUserid} was disqualified from the tournament by ${
            user.name
          } ${reason ? " (" + reason + ")" : ""})`
        );
    ...
    setautostart: function(tournament, user, params, cmd) {
      ...
      if (this.meansYes(option) || option === "start") {
        if (tournament.isTournamentStarted) {
          ...
          return this.errorReply(
            "The tournament does not have a player cap set."
          );
        } else {
          ...
          this.room.add(
            `The tournament will start once ${tournament.playerCap} players have joined.`
          );
          this.privateModAction(
        ...
        if (
          ...
          if (
            !tournament.autostartcap &&
            tournament.autoStartTimeout === Infinity
          )
            return this.errorReply(
              "The automatic tournament start timer is already off."
            );
          params[0] = "off";
          tournament.autostartcap = false;
        }
        const timeout =
          params[0].toLowerCase() === "off" ? Infinity : params[0];
        if (tournament.setAutoStartTimeout(timeout * 60 * 1000, this)) {
          this.privateModAction(
            `(The tournament auto start timer was set to  ${params[0]} by ${
              user.name
            })`
          );
          this.modlog(
    ...
    setautodq: function(tournament, user, params, cmd) {
      if (params.length < 1) {
        if (tournament.autoDisqualifyTimeout !== Infinity) {
          return this.sendReply(
            `Usage: ${cmd} <minutes|off>; The current automatic disqualify timer is set to ${tournament.autoDisqualifyTimeout /
              1000 /
              60} minute(s)`
          );
        } else {
      ...
      if (params[0].toLowerCase() === "infinity" || params[0] === "0")
        params[0] = "off";
      const timeout =
        params[0].toLowerCase() === "off" ? Infinity : params[0] * 60 * 1000;
      if (timeout === tournament.autoDisqualifyTimeout)
        return this.errorReply(
          `The automatic tournament disqualify timer is already set to ${
            params[0]
          } minute(s).`
        );
      if (tournament.setAutoDisqualifyTimeout(timeout, this)) {
        this.privateModAction(
          `(The tournament auto disqualify timer was set to ${params[0]} by ${
            user.name
          })`
        );
        this.modlog(
    ...
    runautodq: function(tournament, user) {
      if (tournament.autoDisqualifyTimeout === Infinity)
        return this.errorReply(
          "The automatic tournament disqualify timer is not set."
        );
    ...
    setscouting: function(tournament, user, params, cmd) {
      ...
      if (this.meansYes(option) || option === "allow" || option === "allowed") {
        if (tournament.scouting)
          return this.errorReply(
            "Scouting for this tournament is already set to allowed."
          );
        tournament.scouting = true;
        tournament.modjoin = false;
        this.room.add("|tournament|scouting|allow");
        this.privateModAction(
          `(The tournament was set to allow scouting by ${user.name})`
        );
        ...
        if (!tournament.scouting)
          return this.errorReply(
            "Scouting for this tournament is already disabled."
          );
        tournament.scouting = false;
        tournament.modjoin = true;
        this.room.add("|tournament|scouting|disallow");
        this.privateModAction(
          `(The tournament was set to disallow scouting by ${user.name})`
        );
    ...
    setmodjoin: function(tournament, user, params, cmd) {
      if (params.length < 1) {
        if (tournament.modjoin) {
          return this.sendReply(
            "This tournament allows players to modjoin their battles."
          );
        } else {
          return this.sendReply(
            "This tournament does not allow players to modjoin their battles."
          );
        }
      }
      ...
      if (this.meansYes(option) || option === "allow" || option === "allowed") {
        if (tournament.modjoin)
          return this.errorReply(
            "Modjoining is already allowed for this tournament."
          );
        tournament.modjoin = true;
        this.room.add(
          "Modjoining is now allowed (Players can modjoin their tournament battles)."
        );
        this.privateModAction(
          `(The tournament was set to allow modjoin by ${user.name})`
        );
        ...
        if (!tournament.modjoin)
          return this.errorReply(
            "Modjoining is already not allowed for this tournament."
          );
        tournament.modjoin = false;
        this.room.add(
          "Modjoining is now banned (Players cannot modjoin their tournament battles)."
        );
        this.privateModAction(
          `(The tournament was set to disallow modjoin by ${user.name})`
        );
        this.modlog("TOUR MODJOIN", null, option);
      } else {
        return this.sendReply(`Usage: ${cmd} <allow|disallow>`);
    ...
    forcetimer: function(tournament, user, params, cmd) {
      ...
      if (this.meansYes(option)) {
        ...
        this.privateModAction(
          `(The timer was turned on for the tournament by ${user.name})`
        );
        ...
        this.privateModAction(
          `(The timer was turned off for the tournament by ${user.name})`
        );
        this.modlog("TOUR FORCETIMER", null, "ON");
      } else {
    ...
    banuser: function(tournament, user, params, cmd) {
      ...
      this.privateModAction(
        `${targetUser.name ||
          targetUserid} was banned from joining tournaments by ${
          user.name
        }.${reason}`
      );
    },
    unbanuser: function(tournament, user, params, cmd) {
      ...
      this.privateModAction(
        `${targetUser.name ||
          targetUserid} was unbanned from joining tournaments by ${user.name}.`
      );
...
Chat.commands.tournament = function(paramString, room, user, connection) {
  ...
  if (cmd === "") {
    if (!this.runBroadcast()) return;
    const update = Object.keys(exports.tournaments)
      .filter(tournament => {
        tournament = exports.tournaments[tournament];
        return (
          !tournament.room.isPrivate &&
          !tournament.room.isPersonal &&
          !tournament.room.staffRoom
        );
      })
      .map(tournament => {
        tournament = exports.tournaments[tournament];
    ...
    if (rank && rank === "@") {
      if (room.toursEnabled === true)
        return this.errorReply(
          "Tournaments are already enabled for @ and above in this room."
        );
      ...
      return this.errorReply(
        "Tournament enable setting not recognized.  Valid options include [%|@]."
      );
    }
    ...
    if (this.meansYes(option)) {
      ...
      this.privateModAction(
        `(Tournament announcements were enabled by ${user.name})`
      );
      this.modlog("TOUR ANNOUNCEMENTS", null, "ON");
    } else if (this.meansNo(option)) {
      if (!room.tourAnnouncements)
        return this.errorReply(
          "Tournament announcements are already disabled."
        );
      room.tourAnnouncements = false;
      this.privateModAction(
        `(Tournament announcements were disabled by ${user.name})`
      );
    ...
    if (params.length < 2) {
      return this.sendReply(
        `Usage: ${cmd} <format>, <type> [, <comma-separated arguments>]`
      );
    }

    let tour = createTournament(
      room,
      params.shift(),
      params.shift(),
      params.shift(),
    ...
    if (tour) {
      this.privateModAction(
        `(${user.name} created a tournament in ${tour.format} format.)`
      );
      ...
      if (room.tourAnnouncements) {
        ...
        if (tourRoom && tourRoom !== room)
          tourRoom
            .addRaw(
              `<div class="infobox"><a href="/${
                room.id
              }" class="ilink"><strong>${Chat.escapeHTML(
                Dex.getFormat(tour.format).name
              )}</strong> tournament created in <strong>${Chat.escapeHTML(
                room.title
              )}</strong>.</a></div>`
            )
            .update();
    ...
    if (!tournament) {
      return this.sendReply(
        "There is currently no tournament running in this room."
      );
    }
    ...
    if (commands.basic[cmd]) {
      commandHandler =
        typeof commands.basic[cmd] === "string"
          ? commands.basic[commands.basic[cmd]]
          : commands.basic[cmd];
    }
    ...
    if (commands.creation[cmd]) {
      ...
      commandHandler =
        typeof commands.creation[cmd] === "string"
          ? commands.creation[commands.creation[cmd]]
          : commands.creation[cmd];
    }
    ...
    if (commands.moderation[cmd]) {
      ...
      commandHandler =
        typeof commands.moderation[cmd] === "string"
          ? commands.moderation[commands.moderation[cmd]]
          : commands.moderation[cmd];
    }
...
Chat.commands.tournamenthelp = function(target, room, user) {
  ...
  return this.sendReplyBox(
    `/tour create/new &lt;format>, &lt;type>, [ &lt;comma-separated arguments>]: Creates a new tournament in the current room.<br />` +
      ...
      `/tour rules/banlist &lt;comma-separated arguments>: Sets the custom rules for the tournament before it has started.<br />` +
      ...
      `/tour on/enable &lt;%|@>: Enables allowing drivers or mods to start tournaments in the current room.<br />` +
      `/tour off/disable: Disables allowing drivers and mods to start tournaments in the current room.<br />` +
      ...
      `More detailed help can be found <a href="https://www.smogon.com/forums/threads/3570628/#post/tour6777489">here</a>`
  );
};
