...
const commands = {
  ...
  authority: function(target, room, user, connection) {
    ...
    let buffer = Object.keys(rankLists)
      .sort(
        (a, b) =>
          (Config.groups[b] || { rank: 0 }).rank -
          (Config.groups[a] || { rank: 0 }).rank
      )
      .map(
        r =>
          `${
            ...
            .sort((a, b) => toId(a).localeCompare(toId(b)))
            .join(", ")}`
      );
  ...
  userlist: function(target, room, user) {
    ...
    let output = `There ${Chat.plural(
      userList,
      "are",
      "is"
    )} <strong style="color:#24678d">${Chat.count(
      userList,
      "</strong> users"
    )} in this room:<br />`;
  ...
  userlisthelp: [
    `/userlist - Displays a list of users who are currently in the room.`
  ],
  ...
  code: function(target, room, user) {
    ...
    if (target.includes(separator) || target.length > 150) {
      ...
      let code = `<div class="chat"><code style="white-space: pre-wrap; display: table">${output.join(
        "<br />"
      )}</code></div>`;
      if (output.length > 3)
        code = `<details><summary>See code...</summary>${code}</details>`;

      if (!this.canBroadcast(true, "!code")) return;
      if (this.broadcastMessage && !this.can("broadcast", null, room))
        return false;
      ...
      return this.errorReply(
        "You can simply use ``[code]`` for code messages that are only one line."
      );
    }
  },
  codehelp: [
  ...
  avatar: function(target, room, user) {
    ...
    let avatarTable = {
      ...
      blackbeltgen4dp: 15,
      artistgen4: 16,
      pokemonbreedergen4: 17,
      pokemonbreederfgen4: 18,
      cowgirl: 19,
      jogger: 20,
      pokefangen4: 21,
      pokefanfgen4: 22,
      pokekid: 23,
      youngcouplegen4dp: 24,
      acetrainergen4dp: 25,
      acetrainerfgen4dp: 26,
      waitressgen4: 27,
      veterangen4: 28,
      ninjaboy: 29,
      dragontamer: 30,
      ...
      collector: 38,
      policemangen4: 39,
      pokemonrangergen4: 40,
      pokemonrangerfgen4: 41,
      scientistgen4dp: 42,
      swimmergen4dp: 43,
      swimmerfgen4dp: 44,
      tuber: 45,
  ...
  avatarhelp: [
    `/avatar [avatar number 1 to 293] - Change your trainer sprite.`
  ],
  ...
  msg: function(target, room, user, connection) {
    ...
    if (this.targetUsername === "~") {
      ...
      error = `|pm|${this.user.getIdentity()}| ${
        this.targetUsername
      }|/error ${error}`;
  ...
  invite: function(target, room, user) {
    ...
    if (room) {
      if (!this.targetUser)
        return this.errorReply(
          `The user "${this.targetUsername}" was not found.`
        );
      if (!targetRoom)
    ...
    if (!targetRoom || targetRoom === Rooms.global)
      return this.errorReply(`The room "${target}" was not found.`);
    if (targetRoom.staffRoom && !targetUser.isStaff)
      return this.errorReply(
        `User "${targetUser.name}" requires global auth to join room "${targetRoom.id}".`
      );
    if (!targetUser)
      return this.errorReply(`The user "${targetUser.name}" was not found.`);
  ...
  pminfobox: function(target, room, user, connection) {
    ...
    if (!(targetUser in room.users) && !user.can("addhtml"))
      return this.errorReply(
        "You do not have permission to use this command to users who are not in this room."
      );
    if (
      targetUser.ignorePMs &&
      targetUser.ignorePMs !== user.group &&
      !user.can("lock")
    )
      return this.errorReply("This user is currently ignoring PMs.");
    if (targetUser.locked && !user.can("lock"))
      return this.errorReply(
        "This user is currently locked, so you cannot send them a pminfobox."
      );
  ...
  pminfoboxhelp: [
    `/pminfobox [user], [html]- PMs an [html] infobox to [user]. Requires * ~`
  ],
  ...
  pmuhtml: function(target, room, user, connection, cmd) {
    ...
    if (targetUser.locked && !user.can("lock"))
      return this.errorReply(
        "This user is currently locked, so you cannot send them UHTML."
      );

    let message = `|pm|${user.getIdentity()}|${targetUser.getIdentity()}|/uhtml${
      (cmd === "pmuhtmlchange" ? "change" : "")
    } ${target}`;
  ...
  ignorepms: function(target, room, user) {
    ...
    if (target in Config.groups) {
      ...
      return this.sendReply(
        `You are now blocking private messages, except from staff and ${target}.`
      );
    }
    return this.sendReply(
  ...
  awayhelp: [
    `/away - Blocks challenges and private messages. Unblock them with /back.`
  ],
  ...
  backhelp: [
    `/back - Unblocks challenges and/or private messages, if either are blocked.`
  ],
  ...
  rank: function(target, room, user) {
    ...
    Ladders.visualizeAll(target).then(values => {
      ...
      buffer += `<tr><td colspan="8">User: <strong>${Chat.escapeHTML(
        target
      )}</strong></td></tr>`;
  ...
  makechatroom: function(target, room, user, connection, cmd) {
    ...
    if (id.length > MAX_CHATROOM_ID_LENGTH)
      return this.errorReply("The given room title is too long.");
    // Check if the name already exists as a room or alias
    if (Rooms.search(id))
      return this.errorReply(`The room '${target}' already exists.`);
    if (!Rooms.global.addChatRoom(target))
      return this.errorReply(
        `An error occurred while trying to create the room '${target}'.`
      );
    ...
    if (cmd === "makeprivatechatroom") {
      ...
      if (Rooms.get("upperstaff")) {
        Rooms.get("upperstaff")
          .add(
            `|raw|<div class="broadcast-green">Private chat room created: <b>${Chat.escapeHTML(
              target
            )}</b></div>`
          )
          .update();
      ...
      if (Rooms.get("staff")) {
        Rooms.get("staff")
          .add(
            `|raw|<div class="broadcast-green">Public chat room created: <b>${Chat.escapeHTML(
              target
            )}</b></div>`
          )
          .update();
  ...
  makechatroomhelp: [
    `/makechatroom [roomname] - Creates a new room named [roomname]. Requires: & ~`
  ],
  ...
  makegroupchat: function(target, room, user, connection, cmd) {
    ...
    if (!user.trusted) {
      return this.errorReply(
        "You must be global voice or roomdriver+ in some public room to make a groupchat."
      );
    }
    if (cmd === "subroomgroupchat") {
      if (!user.can("mute", null, room))
        return this.errorReply(
          "You can only create subroom groupchats for rooms you're staff in."
        );
      if (room.battle)
    ...
    if (title.length >= 32) {
      ...
      let filterResult = Config.chatfilter.call(
        this,
        title,
        user,
        null,
    ...
    if (existingRoom && !existingRoom.modjoin)
      return this.errorReply(`The room '${title}' already exists.`);
    ...
    if (Monitor.countGroupChat(connection.ip)) {
      this.errorReply(
        "Due to high load, you are limited to creating 4 group chats every hour."
      );
      return;
    }
    ...
    if (/^[0-9]+$/.test(title)) {
      ...
      titleHTML = `${Chat.escapeHTML(
        title
      )} <small style="font-weight:normal;font-size:9pt">${groupChatLink}</small>`;
    }
  ...
  deregisterchatroom: function(target, room, user) {
    if (!this.can("makeroom")) return;
    this.errorReply(
      "NOTE: You probably want to use `/deleteroom` now that it exists."
    );
    let id = toId(target);
    ...
    if (Rooms.global.deregisterChatRoom(id)) {
      ...
      if (isPrivate) {
        if (upperStaffRoom)
          upperStaffRoom
            .add(
              `|raw|<div class="broadcast-red">Private chat room deregistered by ${user.userid}: <b>${target}</b></div>`
            )
            .update();
      } else {
        if (staffRoom)
        ...
        if (upperStaffRoom)
          upperStaffRoom
            .add(
              `|raw|<div class="broadcast-red">Public chat room deregistered by ${user.userid}: <b>${target}</b></div>`
            )
            .update();
      }
      return;
  ...
  deleteroom: function(target, room, user, connection, cmd) {
    ...
    if (room.chatRoomData) {
      if (room.isPrivate) {
        if (Rooms.get("upperstaff")) {
          Rooms.get("upperstaff")
            .add(
              Chat.html`|raw|<div class="broadcast-red">Private chat room deleted by ${user.userid}: <b>${title}</b></div>`
            )
            .update();
        }
      } else {
        if (Rooms.get("staff")) {
          Rooms.get("staff")
            .add(
              Chat.html`|raw|<div class="broadcast-red">Public chat room deleted: <b>${title}</b></div>`
            )
            .update();
        }
        if (Rooms.get("upperstaff")) {
          Rooms.get("upperstaff")
            .add(
              Chat.html`|raw|<div class="broadcast-red">Public chat room deleted by ${user.userid}: <b>${title}</b></div>`
            )
            .update();
        }
      }
    ...
    room.add(
      `|raw|<div class="broadcast-red"><b>This room has been deleted.</b></div>`
    );
    ...
    if (this.meansNo(target) || !setting) {
      ...
      if (
        ...
        if (room.privacySetter.size) {
          const privacySetters = Array.from(room.privacySetter).join(", ");
          return this.sendReply(
            `You are no longer forcing the room to stay private, but ${privacySetters} also need${Chat.plural(
              room.privacySetter,
              "",
              "s"
            )} to use /publicroom to make the room public.`
          );
        }
      }
      ...
      if (room.subRooms)
        return this.errorReply("Private rooms cannot have subrooms.");
  ...
  officialroom: function(target, room, user) {
    ...
    if (!room.chatRoomData) {
      return this.errorReply(
        `/officialroom - This room can't be made official`
      );
    }
    if (this.meansNo(target)) {
      if (!room.isOfficial)
        return this.errorReply(`This chat room is already unofficial.`);
      ...
      if (room.isOfficial)
        return this.errorReply(`This chat room is already official.`);
  ...
  psplwinnerroom: function(target, room, user) {
    ...
    if (!room.chatRoomData) {
      return this.errorReply(
        `/psplwinnerroom - This room can't be marked as a PSPL Winner room`
      );
    }
    if (this.meansNo(target)) {
      if (!room.pspl)
        return this.errorReply(
          `This chat room is already not a PSPL Winner room.`
        );
      delete room.pspl;
      this.addModAction(
        `${user.name} made this chat room no longer a PSPL Winner room.`
      );
      ...
      if (room.pspl)
        return this.errorReply("This chat room is already a PSPL Winner room.");
      room.pspl = true;
  ...
  subroom: function(target, room, user) {
    if (!user.can("makeroom"))
      return this.errorReply(
        `/subroom - Access denied. Did you mean /subrooms?`
      );
    ...
    if (main.isPrivate || !main.chatRoomData)
      return this.errorReply(`Only public rooms can have subrooms.`);
    if (room === main)
      return this.errorReply(
    ...
    const mainIdx = Rooms.global.chatRoomDataList.findIndex(
      r => r.title === main.title
    );
    const subIdx = Rooms.global.chatRoomDataList.findIndex(
    ...
    if (mainIdx > subIdx) {
      ...
      Rooms.global.chatRoomDataList[mainIdx] =
        Rooms.global.chatRoomDataList[subIdx];
    ...
    return this.addModAction(
      `This room was set as a subroom of ${main.title} by ${user.name}.`
    );
  },
  ...
  unsubroom: function(target, room, user) {
    ...
    if (!room.parent || !room.chatRoomData)
      return this.errorReply(
        `This room is not currently a subroom of a public room.`
      );
    ...
    return this.addModAction(
      `This room was unset as a subroom by ${user.name}.`
    );
  },
  ...
  subrooms: function(target, room, user, connection, cmd) {
    ...
    const subRoomText = subRooms.map(
      room =>
        Chat.html`<a href="/${room.id}">${room.title}</a><br/><small>${room.desc}</small>`
    );

    return this.sendReplyBox(
      `<p style="font-weight:bold;">${Chat.escapeHTML(
        room.title
      )}'s subroom${Chat.plural(subRooms)}:</p><ul><li>${subRoomText.join(
        "</li><br/><li>"
      )}</li></ul></strong>`
    );
  },
  ...
  roomdesc: function(target, room, user) {
    ...
    if (normalizedTarget.includes(" welcome ")) {
      return this.errorReply(
        `Error: Room description must not contain the word "welcome".`
      );
    }
    if (normalizedTarget.slice(0, 9) === " discuss ") {
      return this.errorReply(
        `Error: Room description must not start with the word "discuss".`
      );
    }
    if (
      normalizedTarget.slice(0, 12) === " talk about " ||
      normalizedTarget.slice(0, 17) === " talk here about "
    ) {
      return this.errorReply(
    ...
    this.privateModAction(
      `(${user.name} changed the roomdesc to: "${target}".)`
    );
  ...
  roomintro: function(target, room, user, connection, cmd) {
    ...
    this.sendReply(
      `|raw|<div class="infobox infobox-limited">${room.introMessage.replace(
        /\n/g,
        ""
      )}</div>`
    );
  ...
  staffintro: function(target, room, user, connection, cmd) {
    if (!target) {
      ...
      if (!room.staffMessage)
        return this.sendReply(
          "This room does not have a staff introduction set."
        );
      this.sendReply(
        `|raw|<div class="infobox">${room.staffMessage.replace(
          /\n/g,
          ``
        )}</div>`
      );
    ...
    this.sendReply(
      `|raw|<div class="infobox">${target.replace(/\n/g, ``)}</div>`
    );
  ...
  roomalias: function(target, room, user) {
    if (!target) {
      if (!this.runBroadcast()) return;
      if (!room.aliases || !room.aliases.length)
        return this.sendReplyBox("This room does not have any aliases.");
      return this.sendReplyBox(
        `This room has the following aliases: ${room.aliases.join(", ")}`
      );
    }
    ...
    if (!alias.length)
      return this.errorReply(
        "Only alphanumeric characters are valid in an alias."
      );
    if (Rooms(alias) || Rooms.aliases.has(alias))
      return this.errorReply(
  ...
  removeroomalias: function(target, room, user) {
    ...
    if (Rooms.aliases.get(alias) !== room.id)
      return this.errorReply(
        "You may only remove an alias from the current room."
      );
  ...
  roomowner: function(target, room, user) {
    ...
    if (target)
      return this.errorReply(
        `This command does not support specifying a reason.`
      );
    let targetUser = this.targetUser;
    ...
    if (targetUser) {
      targetUser.popup(
        `You were appointed Room Owner by ${user.name} in ${room.id}.`
      );
  ...
  roomownerhelp: [
    `/roomowner [username] - Appoints [username] as a room owner. Requires: & ~`
  ],
  ...
  roompromote: function(target, room, user, connection, cmd) {
    ...
    if (!room.auth) {
      this.sendReply(
        "/roompromote - This room isn't designed for per-room moderation."
      );
      return this.sendReply(
        "Before setting room staff, you need to set a room owner with /roomowner"
      );
    }
    if (!this.canTalk()) return;
    ...
    if (!targetUser && !Users.isUsernameKnown(userid) && !force) {
      return this.errorReply(
        `User '${name}' is offline and unrecognized, and so can't be promoted.`
      );
    }
    if (targetUser && !targetUser.registered) {
      return this.errorReply(
        `User '${name}' is unregistered, and so can't be promoted.`
      );
    }
    ...
    if (
      Config.groups[nextGroup].globalonly ||
      (Config.groups[nextGroup].battleonly && !room.battle)
    ) {
      return this.errorReply(
        `Group 'room${Config.groups[nextGroup].id}' does not exist as a room rank.`
      );
    }
    ...
    if ((room.auth[userid] || Config.groupsranking[0]) === nextGroup) {
      return this.errorReply(
        `User '${name}' is already a ${groupName} in this room.`
      );
    }
    if (!user.can("makeroom")) {
      if (
        ...
        if (user.userid !== room.founder) {
          return this.errorReply(
            `/${cmd} - Access denied for promoting/demoting from ${
              (Config.groups[currentGroup]
                ? Config.groups[currentGroup].name
                : "an undefined group")
            }.`
          );
        } else if (user.userid === userid) {
          return this.errorReply(
            `/${cmd} - You cant demote yourself from room founder!`
          );
        } /*
      ...
      if (
        nextGroup !== " " &&
        !user.can("room" + Config.groups[nextGroup].id, null, room)
      ) {
        return this.errorReply(
          `/${cmd} - Access denied for promoting/demoting to ${Config.groups[nextGroup].name}.`
        );
      }
    }
    ...
    if (this.pmTarget && targetUser) {
      ...
      nextGroup in Config.groups &&
      currentGroup in Config.groups &&
      Config.groups[nextGroup].rank < Config.groups[currentGroup].rank
    ) {
      if (
        targetUser &&
        room.users[targetUser.userid] &&
        !Config.groups[nextGroup].modlog
      ) {
      ...
      this.privateModAction(
        `(${name} was demoted to Room ${groupName} by ${user.name}.)`
      );
      this.modlog(`ROOM${groupName.toUpperCase()}`, userid, "(demote)");
      if (needsPopup)
        targetUser.popup(
          `You were demoted to Room ${groupName} by ${user.name} in ${room.id}.`
        );
    } else if (nextGroup === "#") {
      this.addModAction(
        `${"" + name} was promoted to ${groupName} by ${user.name}.`
      );
      this.modlog("ROOM OWNER", userid);
      if (needsPopup)
        targetUser.popup(
          `You were promoted to ${groupName} by ${user.name} in ${room.id}.`
        );
    } else {
      this.addModAction(
        `${"" + name} was promoted to Room ${groupName} by ${user.name}.`
      );
      this.modlog(`ROOM${groupName.toUpperCase()}`, userid);
      if (needsPopup)
        targetUser.popup(
          `You were promoted to Room ${groupName} by ${user.name} in ${room.id}.`
        );
    }
  ...
  roomauth: function(target, room, user, connection, cmd) {
    ...
    if (
      !targetRoom ||
      targetRoom.id === "global" ||
      !targetRoom.checkModjoin(user)
    )
      return this.errorReply(`The room "${target}" does not exist.`);
    if (!targetRoom.auth)
      return this.sendReply(
        `/roomauth - The room '${targetRoom.title ||
          target}' isn't designed for per-room moderation and therefore has no auth list.${userLookup}`
      );
    ...
    let buffer = Object.keys(rankLists)
      ...
      .map(r => {
        ...
        roomRankList = roomRankList.map(s =>
          s in targetRoom.users
            ? Server.nameColor(s, true)
            : Server.nameColor(s)
        );
        return `${
          Config.groups[r] ? `${Config.groups[r].name}s (${r})` : r
    ...
    if (!buffer.length) {
      connection.popup(
        `The room '${targetRoom.title}' has no auth. ${userLookup}`
      );
      return;
    }
    let curRoom = targetRoom;
    while (curRoom.parent) {
      const modjoinSetting =
        curRoom.modjoin === true ? curRoom.modchat : curRoom.modjoin;
      const roomType = (modjoinSetting ? `modjoin ${modjoinSetting} ` : "");
      const inheritedUserType = (modjoinSetting
        ? ` of rank ${modjoinSetting} and above`
        : "");
      if (curRoom.parent) {
        buffer.push(
    ...
    if (targetRoom.founder) {
      buffer.unshift(
        `${
          (targetRoom.founder
            ? `Room Founder:\n${
                Users(targetRoom.founder) && Users(targetRoom.founder).connected
                  ? Server.nameColor(targetRoom.founder, true)
                  : Server.nameColor(targetRoom.founder)
              }`
            : ``)
        }`
      );
    ...
    if (room.autorank)
      buffer.unshift(
        `Autorank is currently set to ${Config.groups[room.autorank].name} (${room.autorank})`
      );
  ...
  ban: function(target, room, user, connection) {
    ...
    if (
      Punishments.getRoomPunishType(room, this.targetUsername) === "BLACKLIST"
    )
      return this.errorReply(
    ...
    if (Punishments.isRoomBanned(targetUser, room.id) && !target) {
      ...
      return this.privateModAction(
        `(${name} would be banned by ${user.name} ${problem}.)`
      );
    }

    if (targetUser.trusted && room.isPrivate !== true && !room.isPersonal) {
      Monitor.log(
        `[CrisisMonitor] Trusted user ${targetUser.name} ${
          (targetUser.trusted !== targetUser.userid
            ? ` (${targetUser.trusted})`
            : ``)
        } was roombanned from ${room.id} by ${
          user.name
        }, and should probably be demoted.`
      );
    }

    if (targetUser in room.users || user.can("lock")) {
      targetUser.popup(
        `|modal||html|<p>${Chat.escapeHTML(
          user.name
        )} has banned you from the room ${room.id} ${
          (room.subRooms ? ` and its subrooms` : ``)
        }.</p>${
          (target ? `<p>Reason: ${Chat.escapeHTML(target)}</p>` : ``)
        }<p>To appeal the ban, PM the staff member that banned you${
          (!room.battle && room.auth
            ? ` or a room owner. </p><p><button name="send" value="/roomauth ${room.id}">List Room Staff</button></p>`
            : `.</p>`)
        }`
      );
    }
    ...
    this.addModAction(
      `${name} was banned from ${room.title} by ${user.name}.${reason}`
    );
    ...
    if (!room.isPrivate && room.chatRoomData) {
      let acAccount =
        (targetUser.autoconfirmed !== userid && targetUser.autoconfirmed);
      let displayMessage = "";
      if (affected.length > 1) {
        displayMessage = `(${name}'s ${
          (acAccount ? ` ac account: ${acAccount}, ` : ``)
        } banned alts: ${affected
          .slice(1)
  ...
  banhelp: [
    `/roomban [username], [reason] - Bans the user from the room you are in. Requires: @ # & ~`
  ],
  ...
  unban: function(target, room, user, connection) {
    ...
    if (name) {
      this.addModAction(
        `${name} was unbanned from ${room.title} by ${user.name}.`
      );
      if (room.isPrivate !== true && room.chatRoomData) {
  ...
  unbanhelp: [
    `/roomunban [username] - Unbans the user from the room you are in. Requires: @ # & ~`
  ],
  ...
  join: function(target, room, user, connection) {
    ...
    if (target.startsWith("play.pokemonshowdown.com/"))
      target = target.slice(25);
  ...
  warn: function(target, room, user) {
    ...
    if (!this.canTalk()) return;
    if (room.isPersonal && !user.can("warn"))
    ...
    if (targetUser.can("makeroom"))
      return this.errorReply(
        "You are not allowed to warn upper staff members."
      );

    this.addModAction(
      `${targetUser.name} was warned by ${user.name}.${
        (target ? ` (${target})` : ``)
      }`
    );
  ...
  redir: function(target, room, user, connection) {
    ...
    if (room.isPrivate || room.isPersonal)
      return this.errorReply(
        "Users cannot be redirected from private or personal rooms."
      );
    ...
    if (
      !this.can("warn", targetUser, room) ||
      !this.can("warn", targetUser, targetRoom)
    )
      return false;
    if (!targetUser || !targetUser.connected) {
      return this.errorReply(`User ${this.targetUsername} not found.`);
    ...
    if (targetUser.inRooms.has(targetRoom.id)) {
      return this.errorReply(
        `User ${targetUser.name} is already in the room ${targetRoom.title}!`
      );
    }
    ...
    targetUser.popup(
      `You are in the wrong room; please go to <<${targetRoom.id}>> instead`
    );
    this.addModAction(
      `${targetUser.name} was redirected to room ${targetRoom.title} by ${user.name}.`
    );
    this.modlog("REDIRECT", targetUser, `to ${targetRoom.title}`, {
      noip: 1,
      noalts: 1
    });
  ...
  mute: function(target, room, user, connection, cmd) {
    ...
    let muteDuration =
      (cmd === "hm" || cmd === "hourmute" ? HOURMUTE_LENGTH : MUTE_LENGTH);
    if (!this.can("mute", targetUser, room)) return false;
    if (targetUser.can("makeroom"))
      return this.errorReply(
        "You are not allowed to mute upper staff members."
      );
    let canBeMutedFurther =
      ((room.getMuteTime(targetUser) || 0) <= (muteDuration * 5) / 6);
    if (
      targetUser.locked ||
      (room.isMuted(targetUser) && !canBeMutedFurther) ||
      Punishments.isRoomBanned(targetUser, room.id)
    ) {
      let problem = ` but was already ${
        (targetUser.locked
          ? "locked"
      ...
      return this.addModAction(
        `${targetUser.name} would be muted by ${user.name} ${problem}. (${target})`
      );
    }

    if (targetUser in room.users)
      targetUser.popup(
        `|modal|${user.name} has muted you in ${
          room.id
        } for ${Chat.toDurationString(muteDuration)}. ${target}`
      );
    this.addModAction(
      `${targetUser.name} was muted by ${user.name} for ${Chat.toDurationString(
        muteDuration
      )}.${(target ? ` (${target})` : ``)}`
    );
    this.modlog(`${cmd.includes("h") ? "HOUR" : ""}MUTE`, targetUser, target);
    if (
      targetUser.autoconfirmed &&
      targetUser.autoconfirmed !== targetUser.userid
    ) {
  ...
  unmute: function(target, room, user) {
    ...
    let targetUser = this.targetUser;
    let successfullyUnmuted = room.unmute(
      targetUser ? targetUser.userid : toId(this.targetUsername),
    ...
    if (successfullyUnmuted) {
      this.addModAction(
        `${
          (targetUser ? targetUser.name : successfullyUnmuted)
        } was unmuted by ${user.name}.`
      );
      this.modlog("UNMUTE", (targetUser || successfullyUnmuted), null, {
        noip: 1,
        noalts: 1
      });
    } else {
      this.errorReply(
        `${(targetUser ? targetUser.name : this.targetUsername)} is not muted.`
      );
    }
  },
  unmutehelp: [
  ...
  lock: function(target, room, user, connection, cmd) {
    ...
    target = this.splitTarget(target);
    let targetUser = this.targetUser;
    if (!targetUser && !Punishments.search(toId(this.targetUsername))[0].length)
      return this.errorReply(`User '${this.targetUsername}' not found.`);
    if (target.length > MAX_REASON_LENGTH) {
      return this.errorReply(
        `The reason is too long. It cannot exceed ${MAX_REASON_LENGTH} characters.`
    ...
    if (targetUser) {
      ...
      if (targetUser.trusted) {
        if (cmd === "forcelock") {
          ...
          Monitor.log(
            `[CrisisMonitor] ${name} was locked by ${
              user.name
            } and demoted from ${from.join(", ")}.`
          );
          this.globalModlog(
            "CRISISDEMOTE",
            targetUser,
          ...
          return this.sendReply(
            `${name} is a trusted user. If you are sure you would like to lock them use /forcelock.`
          );
        }
      ...
      if (roomauth.length)
        Monitor.log(
          `[CrisisMonitor] Locked user ${name} has public roomauth (${roomauth.join(
            ", "
          )}), and should probably be demoted.`
        );
    } else {
    ...
    if (
      target &&
      (targetLowercase.includes("spoiler:") ||
        targetLowercase.includes("spoilers:"))
    ) {
      let proofIndex = (targetLowercase.includes("spoilers:")
        ? targetLowercase.indexOf("spoilers:")
        : targetLowercase.indexOf("spoiler:"));
      let bump = (targetLowercase.includes("spoilers:") ? 9 : 8);
      proof = `(PROOF: ${target
        .substr(proofIndex + bump, target.length)
        .trim()}) `;
    ...
    if (targetUser) {
      ...
      targetUser.send(
        `|popup||html||modal|${
          user.name
        } has locked you from talking in chats, battles, and PMing regular users${weekMsg}.${
          (userReason ? `\n\nReason: ${userReason}` : "")
    ...
    let lockMessage =
      `${name} was locked from talking${weekMsg} by ${user.name}.` +
    ...
    let acAccount =
      (targetUser &&
      targetUser.autoconfirmed !== userid &&
      targetUser.autoconfirmed);
    let displayMessage = "";
    if (affected.length > 1) {
      displayMessage = `(${name}'s ${
  ...
  unlock: function(target, room, user) {
    ...
    if (unlocked) {
      const unlockMessage = `${unlocked.join(", ")} ${
        (unlocked.length > 1 ? "were" : "was")
      ...
      if (!reason)
        this.globalModlog("UNLOCK", toId(target), ` by ${user.userid}`);
  ...
  unlockname: function(target, room, user) {
    ...
    for (const curUser of Users.findUsers([userid], [])) {
      if (
        curUser.locked &&
        !curUser.locked.startsWith("#") &&
        !Punishments.getPunishType(curUser.userid)
      ) {
  ...
  unlockip: function(target, room, user) {
    ...
    this.globalModlog(
      `UNLOCK${range ? "RANGE" : "IP"}`,
      target,
      ` by ${user.name}`
    );

    const broadcastRoom = Rooms("staff") || room;
    broadcastRoom.addByUser(
      user,
  ...
  globalban: function(target, room, user, connection, cmd) {
    ...
    if (targetUser.trusted) {
      if (cmd === "forceglobalban") {
        let from = targetUser.distrust();
        Monitor.log(
          `[CrisisMonitor] ${name} was globally banned by ${
            user.name
          } and demoted from ${from.join(", ")}.`
        );
        this.globalModlog(
    ...
    let banMessage = `${name} was globally banned by ${user.name}.${
      (userReason ? ` (${userReason})` : ``)
    }`;
    ...
    if (affected.length > 1) {
      ...
      affected = affected
        .slice(1)
        .map(user => user.getLastName())
        .filter(alt => alt.substr(0, 7) !== "[Guest ");
      guests -= affected.length;
      displayMessage = `(${name}'s ${
        (acAccount ? `ac account: ${acAccount}, ` : ``)
      } banned alts: ${affected.join(", ")} ${
  ...
  unglobalbanhelp: [
    `/unglobalban [username] - Unban a user. Requires: @ * & ~`
  ],
  ...
  deroomvoiceall: function(target, room, user) {
    ...
    this.addModAction(
      `All ${count} roomvoices have been cleared by ${user.name}.`
    );
    this.modlog("DEROOMVOICEALL");
  },
  deroomvoiceallhelp: [
    `/deroomvoiceall - Devoice all roomvoiced users. Requires: # & ~`
  ],
  ...
  baniphelp: [
    `/banip [ip] - Globally bans this IP or IP range for an hour. Accepts wildcards to ban ranges. Existing users on the IP will not be banned. Requires: & ~`
  ],
  ...
  unbanip: function(target, room, user) {
    ...
    Punishments.ips.delete(target);
    this.addModAction(
      `${user.name} unbanned the ${
        (target.charAt(target.length - 1) === "*" ? "IP range" : "IP")
      }: ${target}`
    );
    this.modlog("UNRANGEBAN", null, target);
  },
  unbaniphelp: [
    `/unbanip [ip] - Unbans. Accepts wildcards to ban ranges. Requires: & ~`
  ],
  ...
  lockip: function(target, room, user) {
    ...
    if (
      curPunishment &&
      (curPunishment[0] === "BAN" || curPunishment[0] === "LOCK")
    ) {
      const punishDesc =
        curPunishment[0] === "BAN"
          ? `temporarily banned`
  ...
  lockiphelp: [
    `/lockip [ip] - Globally locks this IP or IP range for an hour. Accepts wildcards to ban ranges. Existing users on the IP will not be banned. Requires: & ~`
  ],
  ...
  promote: function(target, room, user, connection, cmd) {
    ...
    let currentGroup = ((targetUser && targetUser.group) ||
      Users.usergroups[userid] ||
      " ")[0];
    let nextGroup = target;
    if (target === "deauth") nextGroup = Config.groupsranking[0];
    if (!nextGroup) {
      return this.errorReply(
        "Please specify a group such as /globalvoice or /globaldeauth"
      );
    }
    if (!Config.groups[nextGroup]) {
      return this.errorReply(`Group '${nextGroup}' does not exist.`);
    ...
    if (!cmd.startsWith("global")) {
      ...
      return this.errorReply(
        `Did you mean "/room${groupid}" or "/global${groupid}"?`
      );
    }
    if (
      Config.groups[nextGroup].roomonly ||
      Config.groups[nextGroup].battleonly
    ) {
      return this.errorReply(
        `Group '${nextGroup}' does not exist as a global rank.`
      );
    }
    ...
    if (Config.groups[nextGroup].rank < Config.groups[currentGroup].rank) {
      this.privateModAction(
        `(${name} was demoted to ${groupName} by ${user.name}.)`
      );
  ...
  forcepromote: function(target, room, user) {
    ...
    if (Users.isUsernameKnown(name)) {
      return this.errorReply(
        "/forcepromote - Don't forcepromote unless you have to."
      );
    }
    Users.setOfflineGroup(name, nextGroup);

    this.addModAction(
      `${name} was promoted to ${(Config.groups[nextGroup].name ||
        "regular user")} by ${user.name}.`
    );
    this.modlog(
  ...
  declare: function(target, room, user) {
    ...
    for (let u in room.users) {
      if (Users(u).connected)
        Users(u).sendTo(room, `|notify|${room.title} announcement!|${target}`);
    }
    this.add(
      Chat.html`|raw|<div class="broadcast-blue"><b>${target}</b></div>`
    );
    this.modlog("DECLARE", null, target);
  },
  declarehelp: [
    `/declare [message] - Anonymously announces a message. Requires: # * & ~`
  ],
  ...
  globaldeclare: function(target, room, user) {
    ...
    Users.users.forEach(u => {
      if (u.connected)
        u.send(
          `|pm|~|${u.group}${u.name}|/raw <div class="broadcast-blue"><b>${target}</b></div>`
        );
    });
    this.modlog(`GLOBALDECLARE`, null, target);
  },
  globaldeclarehelp: [
    `/globaldeclare [message] - Anonymously announces a message to every room on the server. Requires: ~`
  ],
  ...
  chatdeclare: function(target, room, user) {
    ...
    Rooms.rooms.forEach((curRoom, id) => {
      if (id !== "global" && curRoom.type !== "battle")
        curRoom
          .addRaw(`<div class="broadcast-blue"><b>${target}</b></div>`)
          .update();
    });
  ...
  notifyrank: function(target, room, user, connection, cmd) {
    ...
    if (cmd === "notifyoffrank") {
      ...
      if (!title)
        title = `${room.title} ${
          (Config.groups[rank].name ? `${Config.groups[rank].name}+ ` : ``)
        }message!`;
      if (!user.can("addhtml")) {
      ...
      if (notification.length > 300)
        return this.errorReply(
          `Notifications should not exceed 300 characters.`
        );
  ...
  forcerename: function(target, room, user) {
    ...
    if (!targetUser) {
      ...
      if (this.targetUser) {
        return this.errorReply(
          `User has already changed their name to '${this.targetUser.name}'.`
        );
      }
    ...
    let entry = `${targetUser.name} was forced to choose a new name by ${
      user.name
    }${(reason ? `: ${reason}` : ``)}`;
    ...
    targetUser.send(
      `|nametaken||${user.name} considers your name inappropriate${
        (reason ? `: ${reason}` : ".")
      }`
    );
  ...
  namelock: function(target, room, user) {
    ...
    if (targetUser.namelocked)
      return this.errorReply(
        `User '${targetUser.name}' is already namelocked.`
      );
  ...
  unnamelock: function(target, room, user) {
    ...
    if (unlocked) {
      this.addModAction(
        `${unlocked} was unnamelocked by ${user.name}.${reason}`
      );
      if (!reason)
        this.globalModlog("UNNAMELOCK", toId(target), ` by ${user.userid}`);
  ...
  hidetext: function(target, room, user, connection, cmd) {
    ...
    let targetUser = this.targetUser;
    let name = this.targetUsername;
    if (!targetUser && !room.log.hasUsername(target))
      return this.errorReply(`User ${target} not found or has no roomlogs.`);
    if (!targetUser && !user.can("lock"))
      return this.errorReply(`User ${name} not found.`);
    let userid = toId(this.inputUsername);
    if (
      !user.can("mute", targetUser, room) &&
      !this.can("ban", targetUser, room)
    )
      return;
    ...
    const localPunished =
      ...
      (targetUser.locked ||
        Punishments.isRoomBanned(targetUser, room.id) ||
        room.isMuted(targetUser)));
    if (!(user.can("lock") || localPunished))
      return this.errorReply(
        `User ${name} is neither locked nor muted/banned from this room.`
      );
    ...
    if (
      ...
      room.sendByUser(
        user,
        `${name}'s alts messages were cleared from ${room.title} by ${user.name}.`
      );
      ...
      room.sendByUser(
        user,
        `${name}'s messages were cleared from ${room.title} by ${user.name}.`
      );
      this.modlog("HIDETEXT", targetUser || userid, null, {
        noip: 1,
        noalts: 1
      });
  ...
  blacklist: function(target, room, user) {
    ...
    if (punishment && punishment[0] === "BLACKLIST") {
      return this.errorReply(
        `This user is already blacklisted from this room.`
      );
    }
      ...
      Monitor.log(
        `[CrisisMonitor] Trusted user ${targetUser.name}${
          (targetUser.trusted !== targetUser.userid
            ? ` (${targetUser.trusted})`
            : "")
        } was blacklisted from ${room.id} by ${
          user.name
        }, and should probably be demoted.`
      );
    }
    ...
    if (targetUser in room.users || user.can("lock")) {
      targetUser.popup(
        `|modal||html|<p>${Chat.escapeHTML(
          user.name
        )} has blacklisted you from the room ${room.id}${
          (room.subRooms ? ` and its subrooms` : "")
        }. Reason: ${Chat.escapeHTML(target)}</p>` +
          `<p>To appeal the ban, PM the staff member that blacklisted you${
            (!room.battle && room.auth
              ? ` or a room owner. </p><p><button name="send" value="/roomauth ${room.id}">List Room Staff</button></p>`
              : `.</p>`)
          }`
    ...
    this.privateModAction(
      `(${name} was blacklisted from ${room.title} by ${user.name}. ${
        (target ? ` (${target})` : "")
      })`
    );

    let affected = Punishments.roomBlacklist(
      room,
      targetUser,
      null,
      null,
  ...
  battleban: function(target, room, user, connection, cmd) {
    ...
    if (
      !room.battle &&
      (!reason.includes(".pokemonshowdown.com/") && cmd !== "forcebattleban")
    ) {
      return this.errorReply(
    ...
    if (Punishments.isBattleBanned(targetUser))
      return this.errorReply(
        `User '${targetUser.name}' is already banned from battling.`
      );
    ...
    targetUser.popup(
      `|modal|${user.name} has prevented you from starting new battles for 2 days${reasonText}`
    );
  ...
  unbattleban: function(target, room, user) {
    ...
    if (unbanned) {
      this.addModAction(
        `${unbanned} was allowed to battle again by ${user.name}.`
      );
  ...
  blacklistname: function(target, room, user) {
    ...
    if (!targetStr || (!reason && REQUIRE_REASONS)) {
      return this.errorReply(
        "Usage: /blacklistname name1, name2, ... | reason"
      );
    }
    ...
    for (const userid of targets) {
      ...
      if (trusted && room.isPrivate !== true) {
        Monitor.log(
          `[CrisisMonitor] Trusted user ${userid}${
            (trusted !== userid ? ` (${trusted})` : ``)
          } was nameblacklisted from ${room.id} by ${
            user.name
          }, and should probably be demoted.`
        );
      }
    ...
    this.privateModAction(
      `(${targets.join(", ")}${Chat.plural(
        targets,
        " were",
        " was"
      )} nameblacklisted from ${room.title} by ${user.name}.)`
    );
    return true;
  },
  blacklistnamehelp: [
    `/blacklistname OR /nameblacklist [username1, username2, etc.] | reason - Blacklists the given username(s) from the room you are in for a year. Requires: # & ~`
  ],
  ...
  unblacklistall: function(target, room, user) {
    ...
    if (!unblacklisted)
      return this.errorReply(
        "No users are currently blacklisted in this room to unblacklist."
      );
    this.addModAction(
      `All blacklists in this room have been lifted by ${user.name}.`
    );
  ...
  showblacklist: function(target, room, user, connection, cmd) {
    ...
    let buf = Chat.html`Blacklist for ${room.title}${
      soonExpiring ? ` (expiring within 3 months)` : ""
    }:<br />`;
    ...
    blMap.forEach((data, userid) => {
      ...
      buf += `- <strong>${userid}</strong>, for ${Chat.count(
        expiresDays,
        "days"
      )}`;
  ...
  markshared: function(target, room, user) {
    ...
    return this.addModAction(
      `The IP '${ip}' was marked as shared by ${user.name}.${note}`
    );
  },
  marksharedhelp: [
    `/markshared [IP], [owner/organization of IP] - Marks an IP address as shared. Note: the owner/organization (i.e., University of Minnesota) of the shared IP is required. Requires @, &, ~`
  ],
  ...
  unmarkshared: function(target, room, user) {
    ...
    if (!Punishments.sharedIps.has(target))
      return this.errorReply("This IP isn't marked as shared.");

    Punishments.removeSharedIp(target);
    this.globalModlog("UNSHAREIP", target, ` by ${user.name}`);
    return this.addModAction(
      `The IP '${target}' was unmarked as shared by ${user.name}.`
    );
  },
  unmarksharedhelp: [
    `/unmarkshared [ip] - Unmarks a shared IP address. Requires @, &, ~`
  ],
    ...
    try {
      if (target === "all") {
        ...
        if (Config.disablehotpatchall)
          return this.errorReply(
            "This server does not allow for the use of /hotpatch all"
          );
        ...
        const processManagers = require("./lib/process-manager")
          .processManagers;
        ...
        Rooms.PM.respawn();
        this.sendReply(
          "Battles have been hot-patched. Any battles started after now will use the new code; however, in-progress battles will continue to use the old code."
        );
        ...
        this.sendReply(
          "The login server has been hot-patched. New login server requests will use the new code."
        );
        ...
        TeamValidatorAsync.PM.respawn();
        this.sendReply(
          "The team validator has been hot-patched. Any battles started after now will have teams be validated according to the new code."
        );
        ...
        this.sendReply(
          "Disabling hot-patch has been moved to its own command:"
        );
      ...
      Rooms.global.notifyRooms(
        ...
        `|c|${user.getIdentity()}|/log ${
          user.name
        } used /hotpatch ${target} - but something failed while trying to hot-patch.`
      );
      return this.errorReply(
  ...
  nohotpatch: function(target, room, user) {
    ...
    const reason = target
      .substr(target.indexOf(separator), target.length)
      .trim();
    ...
    Rooms.global.notifyRooms(
      ...
      `|c|${user.getIdentity()}|/log ${
        user.name
      } has disabled hot-patching ${hotpatch}. Reason: ${reason}`
    );
  },
  nohotpatchhelp: [
  ...
  adddatacenters: function(target, room, user, connection, cmd) {
    ...
    FS("config/datacenters.csv")
      ...
      .then(data => {
        ...
        this.sendReply(
          `Done: ${successes} successes, ${identicals} unchanged.`
        );
  ...
  disableladder: function(target, room, user) {
    ...
    Rooms.rooms.forEach((curRoom, id) => {
      ...
      if (id !== "global")
        curRoom
          .addRaw(`<div class="broadcast-red">${innerHTML}</div>`)
          .update();
    });
    Users.users.forEach(u => {
      if (u.connected)
        u.send(
          `|pm|~|${u.group}${u.name}|/raw <div class="broadcast-red">${innerHTML}</div>`
        );
    });
  },
  ...
  enableladder: function(target, room, user) {
    ...
    Rooms.rooms.forEach((curRoom, id) => {
      if (id !== "global")
        curRoom
          .addRaw(`<div class="broadcast-green">${innerHTML}</div>`)
          .update();
    });
    Users.users.forEach(u => {
      if (u.connected)
        u.send(
          `|pm|~|${u.group}${u.name}|/raw <div class="broadcast-green">${innerHTML}</div>`
        );
    });
  },
  ...
  autolockdownkill: function(target, room, user) {
    ...
    if (this.meansYes(target)) {
      ...
      this.sendReply(
        "The server is now set to automatically kill itself upon the final battle finishing."
      );
      const logRoom = Rooms("staff") || room;
      ...
      this.sendReply(
        "The server is now set to not automatically kill itself upon the final battle finishing."
      );
      const logRoom = Rooms("staff") || room;
  ...
  prelockdown: function(target, room, user) {
    ...
    this.sendReply(
      "Tournaments have been disabled in preparation for the server restart."
    );
    const logRoom = Rooms("staff") || room;
  ...
  kill: function(target, room, user) {
    ...
    if (Rooms.global.lockdown !== true) {
      return this.errorReply(
        "For safety reasons, /kill can only be used during lockdown."
      );
    }

    if (Chat.updateServerLock) {
      return this.errorReply(
        "Wait for /updateserver to finish before using /kill."
      );
    }
  ...
  killhelp: [
    `/kill - restarts the server. Can't be done unless the server is in lockdown state. Requires: ~`
  ],
  ...
  loadbanlist: function(target, room, user, connection) {
    ...
    Punishments.loadBanlist().then(
      ...
      error =>
        connection.sendTo(
          room,
          `Something went wrong while loading ipbans.txt: ${error}`
        )
    );
  },
  loadbanlisthelp: [
  ...
  updateserver: async function(target, room, user, connection) {
    ...
    if (code)
      throw new Error(
        `updateserver: Crash while fetching - make sure this is a Git repository`
      );
    if (!stdout && !stderr) {
    ...
    [code, stdout, stderr] = await exec(
      `git stash save --include-untracked "PS /updateserver autostash"`
    );
  ...
  crashfixed: function(target, room, user) {
    ...
    if (Rooms.lobby) {
      ...
      Rooms.lobby
        .addRaw(
          `<div class="broadcast-green"><b>We fixed the crash without restarting the server!</b><br />You may resume talking in the Lobby and starting new battles.</div>`
        )
        .update();
    }
    const logRoom = Rooms("staff") || room;
  ...
  memoryusage: function(target) {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      results[i] = `${(results[i] / Math.pow(2, 10 * unitIndex)).toFixed(2)} ${
        units[unitIndex]
      }`;
    }
    this.sendReply(
      `||[Main process] RSS: ${results[0]}, Heap: ${results[1]} / ${results[2]}`
    );
  },
  ...
  bashhelp: [
    `/bash [command] - Executes a bash command on the server. Requires: ~ console access`
  ],
  ...
  editbattle: function(target, room, user) {
    ...
    switch (cmd) {
      case "hp":
      case "h":
        room.battle.stream.write(
          `>eval let p=${getPlayer(targets[0]) +
            getPokemon(targets[1])};p.sethp(${parseInt(
            targets[2]
      ...
      case "s":
        room.battle.stream.write(
          `>eval let pl=${getPlayer(targets[0])};let p=pl${getPokemon(
            targets[1]
          )};p.setStatus('${toId(
            targets[2]
          )}');if (!p.isActive){battle.add('','please ignore the above');battle.add('-status',pl.active[0],pl.active[0].status,'[silent]');}`
        );
        break;
      case "pp":
        room.battle.stream.write(
          `>eval let pl=${getPlayer(targets[0])};let p=pl${getPokemon(
            targets[1]
          )};p.moveSlots[p.moves.indexOf('${toId(
            targets[2]
          )}')].pp = ${parseInt(targets[3])};`
        );
        break;
      case "boost":
      case "b":
        room.battle.stream.write(
          `>eval let p=${getPlayer(targets[0]) +
            getPokemon(targets[1])};battle.boost({${toId(
            targets[2]
          )}:${parseInt(targets[3])}},p)`
        );
        break;
      case "volatile":
      case "v":
        room.battle.stream.write(
          `>eval let p=${getPlayer(targets[0]) +
            getPokemon(targets[1])};p.addVolatile('${toId(targets[2])}')`
        );
        break;
      case "sidecondition":
      case "sc":
        room.battle.stream.write(
          `>eval let p=${getPlayer(targets[0])}.addSideCondition('${toId(
            targets[1]
          )}')`
        );
        break;
      ...
      case "fc":
        room.battle.stream.write(
          `>eval battle.addPseudoWeather('${toId(targets[0])}')`
        );
        break;
      case "weather":
      case "w":
        room.battle.stream.write(
          `>eval battle.setWeather('${toId(targets[0])}')`
        );
        break;
      case "terrain":
      case "t":
        room.battle.stream.write(
          `>eval battle.setTerrain('${toId(targets[0])}')`
        );
        break;
      default:
  ...
  allowexportinputlog(
    ...
    if (!battle.allowExtraction)
      return this.errorReply(`Someone must have requested extraction.`);
    ...
    this.addModAction(
      `${user.userid} consents to sharing battle team and choices with ${targetUser.userid}.`
    );
    if (
      battle.allowExtraction.join(",") !==
  ...
  exportinputlog(target, room, user) {
    ...
    if (!battle.inputLog) {
      this.errorReply(
        `This command only works when the battle has ended - if the battle has stalled, ask players to forfeit.`
      );
    ...
    if (battle.allowExtraction[0] !== user.userid) {
      ...
      if (p1)
        p1.sendTo(
          room,
          Chat.html`|html|${user.name} wants to extract the battle input log. <button name="send" value="/allowexportinputlog ${user.userid}">Share your team and choices with "${user.name}"</button>`
        );
    }
  ...
  importinputlog(target, room, user, connection) {
    ...
    if (formatIndex < 0 || nextQuoteIndex < 0)
      return this.errorReply(`Invalid input log.`);
    target = target.replace(/\r/g, "");
    if (
      (`\n` + target).includes(`\n>eval `) &&
      !user.hasConsoleAccess(connection)
    ) {
      return this.errorReply(
    ...
    if (
      nameIndex1 >= 0 &&
      nameNextQuoteIndex1 >= 0 &&
      nameIndex2 >= 0 &&
      nameNextQuoteIndex2 >= 0
    ) {
    ...
    setTimeout(() => {
      ...
      battleRoom.add(
        `|html|<div class="broadcast broadcast-blue"><strong>This is an imported replay</strong><br />Players will need to be manually added with <code>/addplayer</code> or <code>/restoreplayers</code></div>`
      );
    }, 500);
  },
  importinputloghelp: [
  ...
  forfeit: function(target, room, user) {
    if (!room.game)
      return this.errorReply("This room doesn't have an active game.");
  ...
  undo: function(target, room, user) {
    if (!room.game)
      return this.errorReply("This room doesn't have an active game.");
    if (!room.game.undo)
      return this.errorReply("This game doesn't support /undo");
  ...
  addplayer: function(target, room, user) {
    ...
    if (!room.battle)
      return this.errorReply("You can only do this in battle rooms.");
    if (room.rated)
      return this.errorReply("You can only add a Player to unrated battles.");
    ...
    this.addModAction(
      `${name} was added to the battle as Player ${target.slice(1)} by ${
        user.name
      }.`
    );
  ...
  joingame: function(target, room, user) {
    ...
    if (!room.game.joinGame)
      return this.errorReply("This game doesn't support /joingame");
  ...
  leavegame: function(target, room, user) {
    ...
    if (!room.game.leaveGame)
      return this.errorReply("This game doesn't support /leavegame");
  ...
  kickgame: function(target, room, user) {
    if (!room.battle)
      return this.errorReply("You can only do this in battle rooms.");
    if (room.battle.tour || room.battle.rated)
      return this.errorReply(
    ...
    if (room.game.leaveGame(targetUser)) {
      this.addModAction(
        `${targetUser.name} was kicked from a battle by ${user.name} ${
          (target ? ` (${target})` : ``)
        }`
      );
  ...
  timer: function(target, room, user) {
    ...
    // SGgame
    if (toId(room.battle.format) === "gen7wildpokemonalpha")
      return this.errorReply(
        "You can't start the timer during a Wild Pokemon Encounter."
      );
    ...
    if (!target) {
      ...
      return this.sendReply(
        `The game timer is ON (requested by ${[...timer.timerRequesters].join(
          ", "
        )})`
      );
    }
    ...
    if (this.meansNo(target) || target === "stop") {
      if (timer.timerRequesters.size) {
        ...
        if (force)
          room.send(
            `|inactiveoff|Timer was turned off by staff. Please do not turn it back on until our staff say it's okay.`
          );
      } else {
  ...
  search: function(target, room, user, connection) {
    if (target) {
      if (Config.laddermodchat) {
        ...
        if (
          Config.groupsranking.indexOf(userGroup) <
          Config.groupsranking.indexOf(Config.laddermodchat)
        ) {
          let groupName =
            Config.groups[Config.laddermodchat].name || Config.laddermodchat;
          this.popupReply(
            `On this server, you must be of rank ${groupName} or higher to search for a battle.`
          );
          return false;
        }
      }
  ...
  challenge: function(target, room, user, connection) {
    ...
    if (Config.pmmodchat) {
      ...
      if (
        Config.groupsranking.indexOf(userGroup) <
        Config.groupsranking.indexOf(Config.pmmodchat)
      ) {
        let groupName =
          Config.groups[Config.pmmodchat].name || Config.pmmodchat;
        this.popupReply(
          `Because moderated chat is set, you must be of rank ${groupName} or higher to challenge users.`
        );
        return false;
      }
    }
  ...
  allowchallenges: function(target, room, user) {
    if (!user.blockChallenges)
      return this.errorReply("You are already available for challenges!");
  ...
  vtm: function(target, room, user, connection) {
    ...
    let format =
      originalFormat.effectType === "Format"
        ? originalFormat
        : Dex.getFormat("[Gen 7] Pokebank Anything Goes");
    if (format.effectType !== "Format")
      return this.popupReply("Please provide a valid format.");

    TeamValidatorAsync(format.id)
      .validateTeam(user.team)
      .then(result => {
        let matchMessage =
  ...
  crq: function(target, room, user, connection) {
    ...
    if (cmd === "userdetails") {
      ...
      connection.send(
        "|queryresponse|userdetails|" + JSON.stringify(userdetails)
      );
      ...
      if (!trustable) return false;
      connection.send(
        "|queryresponse|rooms|" + JSON.stringify(Rooms.global.getRooms(user))
      );
  ...
  help: function(target, room, user) {
    ...
    if (
      ...
      if (user.group !== Config.groupsranking[0]) {
        this.sendReply(
          "DRIVER COMMANDS: /warn, /mute, /hourmute, /unmute, /alts, /forcerename, /modlog, /modnote, /lock, /unlock, /battleban, /unbattleban, /announce, /redirect"
        );
        this.sendReply("MODERATOR COMMANDS: /ban, /unban, /ip, /modchat");
        this.sendReply(
          "LEADER COMMANDS: /declare, /forcetie, /forcewin, /promote, /demote, /banip, /host, /unbanall"
        );
      }
      ...
      if (typeof allCommands[target] === "string") {
        ...
        targets.length > 1 &&
        typeof allCommands[targets[0]] === "object"
      ) {
        ...
        for (const t of targets) {
          if (!namespace[t])
            return this.errorReply(
              `Help for the command '${target}' was not found. Try /help for general help.`
            );
          namespace = namespace[t];
        }
        if (typeof namespace[helpCmd] === "object")
          return this.sendReply(namespace[helpCmd].join("\n"));
        if (typeof namespace[helpCmd] === "function")
          return this.run(namespace[helpCmd]);
        return this.errorReply(
          `Help for the command '${target}' was not found. Try /help for general help.`
        );
      } else {
