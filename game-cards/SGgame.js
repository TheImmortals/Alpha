...
class SGgame extends Console.Console {
	...
	bag(menu, item) {
		...
		for (let i in player.bag[menu]) {
			if (player.bag[menu][i] > 0) {
				if (item && item === i) {
					output += '<button style="background: #0B9; border: none; border-top: 0.1em solid #001; width: 100%;"><span style="float: left">' + Server.getItem(i).name + '</span><span style="float: right">' + player.bag[menu][i] + '</span></button>';
				} else {
					output += '<button name="send" value="/sggame bag ' + menu + ', ' + i + '" style="background: none; border: none; border-top: 0.1em solid #001; width: 100%;"><span style="float: left">' + Server.getItem(i).name + '</span><span style="float: right">' + player.bag[menu][i] + '</span></button>';
				}
			}
		}
		output += '</div></div>';
	...
	summary(section, pokemon, details) {
		...
		case 'summary':
			...
			let nextLevel = Server.calcExp(pokemon.species, (pokemon.level >= 100 ? 100 : pokemon.level + 1)), curLevel = Server.calcExp(pokemon.species, pokemon.level);
		...
		case 'move':
			...
			for (let i = 0; i < 6; i += 2) {
				...
				if (pmon) {
					...
					output += '<div style="' + Server.getPokemonIcon(pmon.species) + '; width: 35px; height: 50%; display: inline-block; float: left;"></div>';
		...
		case 'learn':
			...
			output += '<b>OT</b>:' + pokemon.ot + '<br/>';
			output += '<b>Level</b>:' + pokemon.level + '<br/>';
			let nextLevel2 = Server.calcExp(pokemon.species, pokemon.level + 1), curLevel2 = Server.calcExp(pokemon.species, pokemon.level);
		...
		case 'use':
			...
			for (let i = 0; i < 6; i += 2) {
				...
				if (pmon2) {
					...
					output += '<div style="' + Server.getPokemonIcon(pmon2.species) + '; width: 35px; height: 50%; display: inline-block; float: left;"></div>';
		...
		case 'pp':
			...
			output += '<b>OT</b>:' + pokemon.ot + '<br/>';
			output += '<b>Level</b>:' + pokemon.level + '<br/>';
			let nextLevel3 = Server.calcExp(pokemon.species, pokemon.level + 1), curLevel3 = Server.calcExp(pokemon.species, pokemon.level);
		...
		default:
			...
			for (let i = 0; i < 6; i += 2) {
				...
				if (mon) {
					...
					output += '<div style="' + Server.getPokemonIcon(mon.species) + '; width: 35px; height: 50%; display: inline-block; float: left;"></div>';
...
class Player {
	constructor(user, starter) {
		...
		this.time = 0;
		this.bag = {items: {leafstone: 1, firestone: 1, waterstone: 1, thunderstone: 1, linkcable: 5, icestone: 1}, medicine: {potion: 5, rarecandy: 3}, pokeballs: {pokeball: 50, greatball: 25, ultraball: 10, masterball: 1}, berries: {oranberry: 5, lumberry: 2}, tms: {}, keyitems: {alphaticket: 1}};
...
exports.commands = {
	...
	playalpha: function (target, room, user, connection, cmd) {
		...
		if (cmd === 'playalpha') {
			...
			try {
				...
				let newPlayer = new Player(user, Dex.fastUnpackTeam(Server.makeWildPokemon(false, false, {name: "ERROR!", species: "Mudkip", level: 10, ability: 0})));
	...
	sggame: {
		...
		evo: function (target, room, user) {
			...
			if (!target) {
				...
				obj.party[Number(action[1])].exp = Server.calcExp(action[2], pokemon.level);
				let aSlot = '0';
				...
				let nMoves = Server.getNewMoves(Dex.getTemplate(pokemon.species), (pokemon.level - 1), pokemon.level, pokemon.moves, action[1], true);
				...
				if (user.console.shed && obj.party.length < 5 && obj.bag.pokeballs.pokeball > 0) {
					...
					let shed = {species: 'shedinja', level: pokemon.level, exp: Server.calcExp('shedinja', pokemon.level), ot: obj.userid, ivs: pokemon.ivs, evs: pokemon.evs, nature: pokemon.nature, ability: "Wonder Guard", happiness: 70, pokeball: pokemon.pokeball};
					...
					if (inBattle) {
						...
						if (item.use.level) {
							...
							if (player.party[target[3]] && player.party[target[3]].level < 100) {
								...
								player.party[target[3]].exp = Server.calcExp(player.party[target[3]].species, lvl);
								user.console.queue.push('text|' + player.party[target[3]].name + ' was elevated to level ' + player.party[target[3]].level + '!');
								// New Moves
								let canReturn = true;
								let nMoves = Server.getNewMoves(pokemon, olvl, lvl, player.party[target[3]].moves, target[3]);
								if (nMoves.length) {
									user.console.queue = user.console.queue.concat(nMoves);
									canReturn = false;
								}
								// Evolution
								let evos = Server.canEvolve(player.party[target[3]], "level", user.userid, {location: null}); // TODO locations
								if (evos) {
									evos = evos.split('|');
						...
						if (item.use.triggerEvo) {
							...
							let evos = Server.canEvolve(player.party[target[3]], trigger, user.userid, {location: null, item: item.id}); // TODO locations
							if (evos) {
								if (trigger === 'trade') {
									let obj = Db.players.get(user.userid);
		...
		battle: function (target, room, user, connection) {
			...
			if (!toId(target)) {
				...
				case 'wild':
					if (!target[1]) {
						Users('sgserver').wildTeams[user.userid] = Server.makeWildPokemon(false, Server.teamAverage(Db.players.get(user.userid).party));
				...
				case 'trainer':
					if (!target[1]) {
						Users('sgserver').trainerTeams[user.userid] = Server.makeComTeam(Server.teamAverage(Db.players.get(user.userid).party), Db.players.get(user.userid).party.length);
	...
	pickstarter: function (target, room, user, connection, cmd) {
		...
		case 'confirmpickstarter':
			let player = new Player(user, Dex.fastUnpackTeam(Server.makeWildPokemon(false, false, {species: target, level: 10, ability: 0, ot: user.userid})));
