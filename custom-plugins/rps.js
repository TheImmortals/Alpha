...
class RPSGame {
	...
	parseWin(winner, loser, inactivity) {
		...
		if (this.gameType === "bucks") {
			...
			//do rank change
			let winnerPoints = Ad("rpsrank").get(winner.userid, 1000);
			let loserPoints = Ad("rpsrank").get(loser.userid, 1000);
			...
			Ad("rpsrank").set(winner.userid, winnerFinalPoints);
			...
			Ad("rpsrank").set(loser.userid, loserFinalPoints);
...
exports.commands = {
	rps: {
		...
		rank: function (target, room, user) {
			...
			let userRank = Ad("rpsrank").get(toId(target), 1000);
		...
		ladder: function (target, room, user) {
			...
			let index = 1;
			let table = Object.keys(Ad("rpsrank").object()).sort(function (a, b) {
				if (Ad("rpsrank").get(a, 1000) > Ad("rpsrank").get(b, 1000)) return -1;
				return 1;
			}).slice(0, 100).map(function (u) {
				return '<tr><td>&nbsp;' + index++ + '&nbsp;</td><td>&nbsp;' + u + '&nbsp;</td><td>&nbsp;' + Ad("rpsrank").get(u, 1000) + "&nbsp;</td></tr>";
			}).join("");
