...
class Dice {
  constructor(room, amount, starter) {
    ...
    this.timer = setTimeout(() => {
      this.room
        .add(
          "|uhtmlchange|" +
            this.room.diceCount +
            '|<div class = "infobox">(This game of dice has been ended due to inactivity.)</div>'
        )
        .update();
    ...
    this.startMessage =
      '<div class="infobox"><b style="font-size: 14pt; color: #24678d"><center>' +
      Server.nameColor(starter, true) +
      ' has started a game of dice for <span style = "color: green">' +
      amount +
      "</span> " +
      buck +
      "!</center></b><br>" +
      ...
      '<button style="margin: 1px 6px ; padding: 4px ; color: white ; border: solid 3px red ; background-image: url(&quot;https://i.ibb.co/6yVKmh1/image.png&quot;) ; text-align: center ; text-shadow: 0 -1px 0 hsl(210 , 40% , 10%) ; border-radius: 30px 30px ; width: 120px" name="send" value="/joindice"><b>Join</b> </button></center>';
    this.room
      .add(
        "|uhtml|" + ++this.room.diceCount + "|" + this.startMessage + "</div>"
      )
      .update();
  }
  ...
  join(user, self) {
    if (this.players.length >= 2)
      return self.errorReply(
        "Two users have already joined this game of dice."
      );
    Economy.readMoney(user.userid, money => {
      if (money < this.bet)
        return self.sendReply(
          "You don't have enough money for this game of dice."
        );
      if (this.players.includes(user))
        return self.sendReply("You have already joined this game of dice.");
      if (this.players.length && this.players[0].latestIp === user.latestIp)
        return self.errorReply(
          "You have already joined this game of dice under the alt '" +
            this.players[0].name +
            "'."
        );
      if (this.players.length >= 2)
      ...
      this.room
        .add(
          "|uhtmlchange|" +
            ...
            this.startMessage +
            "<center>" +
            Server.nameColor(user.name) +
            " has joined the game!</center></div>"
        )
        .update();
  ...
  leave(user, self) {
    if (!this.players.includes(user))
      return self.sendReply("You haven't joined this game of dice yet.");
    if (this.players.length >= 2)
      return self.errorReply(
        "You cannot leave a game of dice once it has been started."
      );
    this.players.splice(this.players.indexOf(user), 1);
    this.room.add(
      "|uhtmlchange|" + this.room.diceCount + "|" + this.startMessage + "</div>"
    );
  }
    ...
    Economy.readMoney(p1.userid, money1 => {
      Economy.readMoney(p2.userid, money2 => {
        if (money1 < this.bet || money2 < this.bet) {
          ...
          user.sendTo(
            this.room,
            "You have been removed from this game of dice, as you do not have enough money."
          );
          other.sendTo(
            this.room,
            user.name +
              " has been removed from this game of dice, as they do not have enough money. Wait for another user to join."
          );
          this.players.splice(this.players.indexOf(user), 1);
          this.room
            .add(
              "|uhtmlchange|" +
                ...
                this.startMessage +
                "<center>" +
                this.players.map(user => Server.nameColor(user.name)) +
                " has joined the game!</center>"
            )
            .update();
          return;
        }
        let players = this.players
          .map(user => Server.nameColor(user.name))
          .join(" and ");
        this.room
          .add(
            "|uhtmlchange|" +
              ...
              "<center>" +
              players +
              " have joined the game!</center></div>"
          )
          .update();
        let roll1, roll2;
        do {
          ...
          this.room
            .add(
              "|uhtmlchange|" +
                ...
                players +
                " have joined the game!<br /><br />" +
                "The game has been started! Rolling the dice...<br />" +
                '<img src = "' +
                diceImg(roll1) +
                '" align = "left" title = "' +
                Server.nameColor(p1.name) +
                '\'s roll"><img src = "' +
                diceImg(roll2) +
                '" align = "right" title = "' +
                Server.nameColor(p2.name) +
                "'s roll\"><br />" +
                Server.nameColor(p1.name, true) +
                " rolled " +
                (roll1 + 1) +
                "!<br />" +
                Server.nameColor(p2.name, true) +
                " rolled " +
                (roll2 + 1) +
                "!<br />" +
                Server.nameColor(winner.name, true) +
                ' has won <b style="color:green">' +
                (this.bet - taxedAmt) +
                "</b> " +
                buck +
                "!<br />" +
                "Better luck next time, " +
                Server.nameColor(loser.name) +
                "!"
            )
            .update();
    ...
    if (user)
      this.room
        .add(
          "|uhtmlchange|" +
            this.room.diceCount +
            '|<div class = "infobox">(This game of dice has been forcibly ended by ' +
...
exports.commands = {
  ...
  dicegame: function(target, room, user) {
    if (room.id === "lobby")
      return this.errorReply("This command cannot be used in the Lobby.");
    if (!user.can("broadcast", null, room) && room.id !== "casino")
      return this.errorReply(
        "You must be ranked + or higher in this room to start a game of dice outside the Casino."
      );
    if (!this.canTalk()) return;
    if (room.dice)
      return this.errorReply(
        "There is already a game of dice going on in this room."
      );

    let amount = Number(target) || 1;
    if (isNaN(target))
      return this.errorReply('"' + target + "\" isn't a valid number.");
    if (target.includes(".") || amount < 1 || amount > 5000)
      return this.sendReply(
        "The number of " +
          currencyPlural +
          " must be between 1 and 5,000 and cannot contain a decimal."
      );
    Economy.readMoney(user.userid, money => {
      if (money < amount)
        return this.sendReply(
  ...
  joindice: function(target, room, user) {
    ...
    if (!this.canTalk()) return;
    if (!room.dice)
      return this.errorReply("There is no game of dice going on in this room.");
  ...
  enddice: function(target, room, user) {
    ...
    if (!room.dice)
      return this.errorReply("There is no game of dice going on in this room.");
    if (!user.can("broadcast", null, room) && !room.dice.players.includes(user))
      return this.errorReply(
        "You must be ranked + or higher in this room to end a game of dice."
      );
