...
function generateNews() {
	...
	for (let i = 0; i < keys.length; i++) {
		...
		newsDisplay.push(`<h4>${keys[i]}</h4>${newsData[1]}<br /><br />—${Server.nameColor(newsData[0], true)} <small>on ${newsData[2]}</small>`);
	}
	return newsDisplay;
}
...
Server.showNews = function (userid, user) {
	...
	if (newsDisplay.length > 0) {
		...
		return user.send(`|pm| ${serverName} Server|${user.getIdentity()}|/raw ${newsDisplay}`);
	}
};
...
exports.commands = {
	...
	serverannouncements: {
		...
		view: function (target, room, user) {
			...
			let output = `<center><strong>${serverName} News:</strong></center>${generateNews().join(`<hr>`)}${showSubButton(user.userid)}`;
		...
		subscribe: function (target, room, user) {
			...
			if (Db.NewsSubscribers.has(user.userid)) return this.errorReply("You are alreading subscribing " + serverName + " News.");
			Db.NewsSubscribers.set(user.userid, true);
			this.sendReply("You have subscribed " + serverName + " News.");
			this.popupReply("|wide||html|You will receive " + serverName + " News automatically once you connect to the " + serverName + " next time.<br><hr><center><button class='button' name='send' value ='/news'>View News</button></center>");
		},
		unsubscribe: function (target, room, user) {
			if (!user.named) return this.errorReply('You must choose a name before unsubscribing');
			if (!Db.NewsSubscribers.has(user.userid)) return this.errorReply("You have not subscribed " + serverName + " News.");
			Db.NewsSubscribers.remove(user.userid);
			this.sendReply("You have unsubscribed " + serverName + " News.");
			this.popupReply("|wide||html|You will no longer automatically receive " + serverName + " News.<br><hr><center><button class='button' name='send' value='/news'>View News</button></center>");
		},
	},
	serverannouncementshelp: ["/news view - Views current " + serverName + " news.",
		"/news delete [news title] - Deletes announcement with the [title]. Requires @, &, ~",
		"/news add [news title], [news desc] - Adds news [news]. Requires @, &, ~",
		"/news subscribe - Subscribes to " + serverName + "News.",
		"/news unsubscribe - Unsubscribes to " + serverName + " News.",
	],
};
