...
function getLastSeen(userid) {
  if (Users(userid) && Users(userid).connected)
    return `<font color = "limegreen"><strong>Currently Online</strong></font>`;
  let seen = Db.seen.get(userid);
  if (!seen) return `<font color = "red"><strong>Never</strong></font>`;
  return (
    Chat.toDurationString(Date.now() - seen, { precision: true }) + " ago."
  );
}
...
exports.commands = {
  dev: {
    give: function(target, room, user) {
      ...
      if (devUsername.length > 18)
        return this.errorReply("Usernames cannot exceed 18 characters.");
      if (isDev(devUsername))
        return this.errorReply(`${target} is already a DEV user.`);
      Db.devs.set(devUsername, 1);
      this.sendReply(
        `|html|${Server.nameColor(target, true)} has been given DEV status.`
      );
      if (Users.get(devUsername))
        Users(devUsername).popup(
          `|html|You have been given DEV status by ${Server.nameColor(
            user.name,
            true
          )}.`
        );
    ...
    take: function(target, room, user) {
      ...
      if (devUsername.length > 18)
        return this.errorReply("Usernames cannot exceed 18 characters.");
      if (!isDev(devUsername))
        return this.errorReply(`${target} isn't a DEV user.`);
      Db.devs.remove(devUsername);
      this.sendReply(
        `|html|${Server.nameColor(
          target,
          true
        )} has been demoted from DEV status.`
      );
      if (Users.get(devUsername))
        Users(devUsername).popup(
          `|html|You have been demoted from DEV status by ${Server.nameColor(
            user.name,
            true
          )}.`
        );
    ...
    users: "list",
    list: function() {
      if (!Db.devs.keys().length)
        return this.errorReply("There seems to be no user(s) with DEV status.");
      let display = [];
      Db.devs.keys().forEach(devUser => {
        display.push(
          Server.nameColor(devUser, Users(devUser) && Users(devUser).connected)
        );
      });
      this.popupReply(
        `|html|<strong><u><font size="3"><center>DEV Users:</center></font></u></strong>${Chat.toListString(
          display
        )}`
      );
    },
  ...
  vip: {
    give: function(target, room, user) {
      ...
      if (vipUsername.length > 18)
        return this.errorReply("Usernames cannot exceed 18 characters.");
      if (isVIP(vipUsername))
        return this.errorReply(`${target} is already a VIP user.`);
      Db.vips.set(vipUsername, 1);
      this.sendReply(
        `|html|${Server.nameColor(
          vipUsername,
          true
        )} has been given VIP status.`
      );
      if (Users.get(vipUsername))
        Users(vipUsername).popup(
          `|html|You have been given VIP status by ${Server.nameColor(
            user.name,
            true
          )}.`
        );
    ...
    take: function(target, room, user) {
      ...
      if (vipUsername.length > 18)
        return this.errorReply("Usernames cannot exceed 18 characters.");
      if (!isVIP(vipUsername))
        return this.errorReply(`${target} isn't a VIP user.`);
      Db.vips.remove(vipUsername);
      this.sendReply(
        `|html|${Server.nameColor(
          vipUsername,
          true
        )} has been demoted from VIP status.`
      );
      if (Users.get(vipUsername))
        Users(vipUsername).popup(
          `|html|You have been demoted from VIP status by ${Server.nameColor(
            user.name,
            true
          )}.`
        );
    ...
    users: "list",
    list: function() {
      if (!Db.vips.keys().length)
        return this.errorReply("There seems to be no user(s) with VIP status.");
      let display = [];
      Db.vips.keys().forEach(vipUser => {
        display.push(
          Server.nameColor(vipUser, Users(vipUser) && Users(vipUser).connected)
        );
      });
      this.popupReply(
        `|html|<strong><u><font size="3"><center>VIP Users:</center></font></u></strong>${Chat.toListString(
          display
        )}`
      );
    },
  ...
  customtitle: {
    ...
    give: function(target, room, user) {
      if (!this.can("profile")) return false;
      let [userid, titleName, color] = target.split(",").map(p => {
        return p.trim();
      });
      ...
      if (color.charAt(0) !== "#")
        return this.errorReply(
          `The color needs to be a hex starting with "#".`
        );
      ...
      if (Users(userid))
        Users(userid).popup(
          `|html|You have received a custom title from ${Server.nameColor(
            user.name,
            true
          )}.<br />Title: <font color="${color}">(<strong>${titleName}</strong>)</font><br />Title Hex Color: ${color}`
        );
      this.privateModAction(
        `${user.name} set a custom title to ${userid}'s profile.`
      );
      Monitor.log(`${user.name} set a custom title to ${userid}'s profile.`);
      return this.sendReply(
        `Title "${titleName}" and color "${color}" for ${userid}'s custom title have been set.`
      );
    },
    ...
    remove: function(target, room, user) {
      ...
      let profile = Db.profile.get(userid, { data: { title: {}, music: {} } });
      if (!(profile.data.title.title || profile.data.title.color))
        return this.errorReply(`${target} doesn't have a custom title yet.`);
  ...
  friendcode: {
    ...
    nintendoswitch: {
      ...
      set: function(target, room, user) {
        if (room.battle)
          return this.errorReply(
            "Please use this command outside of battle rooms."
          );
        if (!user.autoconfirmed)
          return this.errorReply(
            "You must be autoconfirmed to use this command."
          );
        ...
        if (isNaN(fc)) {
          return this.errorReply(
            "Your Switch friend code needs to contain only numerical characters (the SW- will be automatically added)."
          );
        }
        if (fc.length < 12)
          return this.errorReply(
            "Your Switch friend code needs to be 12 digits long."
          );
        fc = `${fc.slice(0, 4)}-${fc.slice(4, 8)}-${fc.slice(8, 12)}`;
        Db.switchfc.set(user.userid, fc);
        return this.sendReply(
          `Your Switch friend code: ${fc} has been saved to the server.`
        );
      },
      ...
      delete: function(target, room, user) {
        ...
        if (!target) {
          if (!Db.switchfc.has(user.userid))
            return this.errorReply("Your friend code isn't set.");
          Db.switchfc.remove(user.userid);
          return this.sendReply(
            "Your Switch friend code has been deleted from the server."
          );
        } else {
          if (!this.can("profile")) return false;
          let userid = toId(target);
          if (!Db.switchfc.has(userid))
            return this.errorReply(`${target} hasn't set a friend code.`);
          Db.switchfc.remove(userid);
          return this.sendReply(
            `${target}'s Switch friend code has been deleted from the server.`
          );
        }
      }
    },
    ...
    ds: {
      ...
      set: function(target, room, user) {
        ...
        if (isNaN(fc)) {
          return this.errorReply(
            "Your friend code needs to contain only numerical characters."
          );
        }
        if (fc.length < 12)
          return this.errorReply(
            "Your friend code needs to be 12 digits long."
          );
        fc = `${fc.slice(0, 4)}-${fc.slice(4, 8)}-${fc.slice(8, 12)}`;
        Db.friendcode.set(user.userid, fc);
        return this.sendReply(
          `Your friend code: ${fc} has been saved to the server.`
        );
      },
      ...
      delete: function(target, room, user) {
        ...
        if (!target) {
          if (!Db.friendcode.has(user.userid))
            return this.errorReply("Your friend code isn't set.");
          Db.friendcode.remove(user.userid);
          return this.sendReply(
            "Your friend code has been deleted from the server."
          );
        } else {
          if (!this.can("profile")) return false;
          let userid = toId(target);
          if (!Db.friendcode.has(userid))
            return this.errorReply(`${target} hasn't set a friend code.`);
          Db.friendcode.remove(userid);
          return this.sendReply(
            `${target}'s friend code has been deleted from the server.`
          );
        }
      }
    },
  ...
  type: {
    ...
    set: function(target, room, user) {
      ...
      let profile = Db.profile.get(user.userid, {
        data: { title: {}, music: {} }
      });
      let type = Dex.getType(target);
      if (!type.exists)
        return this.errorReply("Not a type. Check your spelling?");
    ...
    delete: function(target, room, user) {
      ...
      if (!profile.type)
        return this.errorReply(`Your profile type hasn't been set yet.`);
      delete profile.type;
      Db.profile.set(user.userid, profile);
      return this.sendReply(
        "Your favorite type has been deleted from your profile."
      );
    },
  ...
  pcolor: {
    ...
    remove: function(target, room, user) {
      ...
      if (!target) return this.parse("/pcolor help");
      if (!profile.color)
        return this.errorReply(`${target} does not have a profile color set.`);
      ...
      if (Users(userid))
        Users(userid).popup(
          `|html|${Server.nameColor(
            user.name,
            true
          )} has removed your profile color.`
        );
  ...
  background: {
    ...
    setbg: function(target) {
      if (!this.can("profile")) return false;
      let [userid, link] = target.split(",").map(p => {
        return p.trim();
      });
      userid = toId(userid);
      let profile = Db.profile.get(userid, { data: { title: {}, music: {} } });
      if (!link) return this.parse(`/help background`);
      if (![".png", ".gif", ".jpg"].includes(link.slice(-4)))
        return this.errorReply(
          `Backgrounds must end in an extension like .png, .gif, or .jpg.`
        );
      profile.background = link;
      Db.profile.set(userid, profile);
      this.sendReplyBox(
        `This user's background has been set as:<br /><img src="${link}">`
      );
    },
    ...
    deletebg: function(target) {
      ...
      if (!profile.background)
        return this.errorReply(`${target} doesn't have a custom background.`);
      delete profile.background;
      Db.profile.set(targ, profile);
      return this.sendReply(
        `${target}'${
          target.endsWith("s") ? `` : `s`
        } custom background was removed.`
      );
    },
  ...
  music: {
    ...
    set: function(target) {
      if (!this.can("profile")) return false;
      let [userid, link, title] = target.split(",").map(p => {
        return p.trim();
      });
      userid = toId(userid);
      let profile = Db.profile.get(userid, { data: { title: {}, music: {} } });
      if (!title) return this.parse("/musichelp");
      if (![".mp3", ".mp4", ".m4a"].includes(link.slice(-4)))
        return this.errorReply(
          `Music links must end in an extension like .mp3, .mp4, or .m4a.`
        );
      ...
      this.sendReplyBox(
        `${userid}'${
          userid.endsWith("s") ? `` : `s`
        } song has been set.<br /><acronym title="${
          profile.data.music.title
        }"><br /><audio src="${
          profile.data.music.link
        }" controls="" style="width:100%;"></audio></acronym>`
      );
    },
    ...
    delete: function(target) {
      ...
      if (!(profile.data.music.link || profile.data.music.title))
        return this.errorReply(`${target} does not have any profile music.`);
      delete profile.data.music.link;
      delete profile.data.music.title;
      Db.profile.set(target, profile);
      return this.sendReply(
        `You have removed ${target}'${
          target.endsWith("s") ? `` : `s`
        } profile music.`
      );
    },
  ...
  pokemon: {
    ...
    set: function(target, room, user) {
      ...
      Db.profile.set(user.userid, profile);
      return this.sendReply(
        `You have successfully set your favorite Pokemon as "${pkmn.species}".`
      );
    },
    ...
    delete: function(target, room, user) {
      ...
      if (!profile.pokemon)
        return this.errorReply("Your favorite Pokemon hasn't been set.");
      delete profile.pokemon;
      Db.profile.set(user.userid, profile);
      return this.sendReply(
        "Your favorite Pokemon has been deleted from your profile."
      );
    },
  ...
  nature: {
    ...
    set: function(target, room, user) {
      ...
      if (!nature.exists)
        return this.errorReply("This is not a nature. Check your spelling?");
      profile.nature = nature.name;
      Db.profile.set(user.userid, profile);
      return this.sendReply(
        "You have successfully set your nature onto your profile."
      );
    },
    ...
    delete: function(target, room, user) {
      ...
      if (!profile.nature)
        return this.errorReply("Your nature has not been set.");
  ...
  profile: function(target, room, user) {
    ...
    let profile = Db.profile.get(userid, { data: { title: {}, music: {} } });
    let avatar = targetUser
      ? isNaN(targetUser.avatar)
        ? `http://${serverIp}/avatars/${targetUser.avatar}`
        : `http://play.pokemonshowdown.com/sprites/trainers-custom/${targetUser.avatar}.png`
      : Config.customavatars[userid]
      ? `http://${serverIp}/avatars/${Config.customavatars[userid]}`
      : `http://play.pokemonshowdown.com/sprites/trainers-custom/1.png`;
    if (targetUser && targetUser.avatar[0] === "#")
      avatar = `http://play.pokemonshowdown.com/sprites/trainers-custom/${targetUser.avatar.substr(
        1
      )}.png`;
    let userSymbol = Users.usergroups[userid]
      ? Users.usergroups[userid].substr(0, 1)
      : "Regular User";
    let userGroup = Config.groups[userSymbol]
      ? `Global ${Config.groups[userSymbol].name}`
      : `Regular User`;
    ...
    profileData += `<div style="display: inline-block">&nbsp;${pColor(
      userid
    )}<strong>Name:</strong></font> ${Server.nameColor(username, true)}&nbsp;`;
    if (Users(userid) && ip && ip !== null) {
      profileData += ` <img src="http://flags.fmcdn.net/data/flags/normal/${ip.country.toLowerCase()}.png" alt="${
        ip.country
      }" title="${ip.country}" width="20" height="10">`;
    }
    if (profile.data.title.title)
      profileData += ` <font color="${profile.data.title.color}">(<strong>${profile.data.title.title}</strong>)</font>`;
    profileData += `<br />`;
    profileData += `&nbsp;${pColor(
      userid
    )}<strong>Group:</strong> ${userGroup}</font>`;
    if (isDev(userid))
      profileData += ` || <font color="#009320"><strong>Developer</strong></font>`;
    if (isVIP(userid))
      profileData += ` || <font color="#6390F0"><strong>VIP</strong></font>`;
    profileData += `<br />`;
    profileData += `&nbsp;${pColor(
      userid
    )}<strong>Registered:</strong> ${regdate}</font><br />`;
    profileData += `&nbsp;${pColor(
      userid
    )}<strong>${currencyPlural}:</strong> ${Economy.readMoney(
      userid
    ).toLocaleString()}</font><br />`;
    if (profile.pokemon)
      profileData += `&nbsp;${pColor(
        userid
      )}<strong>Favorite Pokemon:</strong> ${profile.pokemon}</font><br />`;
    if (profile.type)
      profileData += `&nbsp;${pColor(
        userid
      )}<strong>Favorite Type:</strong></font> <img src="https://www.serebii.net/pokedex-bw/type/${
        profile.type
      }.gif"><br />`;
    if (profile.nature)
      profileData += `&nbsp;${pColor(userid)}<strong>Nature:</strong> ${
        profile.nature
      }</font><br />`;
    profileData += `&nbsp;${pColor(
      userid
    )}<strong>EXP Level:</strong> ${Server.ExpControl.level(
      userid
    )}</font><br />`;
    profileData += `&nbsp;${pColor(
      userid
    )}<strong>Last Seen:</strong> ${getLastSeen(userid)}</font><br />`;
    if (Db.friendcode.has(userid))
      profileData += `&nbsp;${pColor(
        userid
      )}<strong>Friend Code:</strong> ${Db.friendcode.get(
        userid
      )}</font><br />`;
    if (Db.switchfc.has(userid))
      profileData += `&nbsp;${pColor(
        userid
      )}<strong>Switch Friend Code:</strong> SW-${Db.switchfc.get(
        userid
      )}</font><br />`;
    if (profile.data.music.link)
      profileData += `&nbsp;<acronym title="${profile.data.music.title}"><br /><audio src="${profile.data.music.link}" controls="" style="width: 100%;"></audio></acronym><br />`;
    profileData += `</div>`;
