...
exports.commands = {
	...
	guilds: {
		...
		decline: function (target, room, user) {
			...
			leagues[leagueid].pendingInvites.splice(leagues[leagueid].pendingInvites.indexOf(user.userid), 1);
			save();
			leagueLog(user.name + " has declined their invitation to join the region.", leagueid);
			leaguePM(Server.ameColor(user.name, true) + " has declined their invitation to join the region.", leagueid);
		...
		points: {
			...
			log: function (target, room, user, connection, cmd) {
				...
				database.all("SELECT * FROM points WHERE " + (cmd === 'userlog' ? "userid=$userid " : "league=$leagueid ") + "ORDER BY date DESC LIMIT 500", searchObj, (err, rows) => {
					if (err) return console.log("/region points log: " + err);
					if (rows.length < 1) return user.popup("No region point logs found for " + Chat.escapeHTML(leagues[leagueid].name));

					let output = '<center>Displaying last 500 entries in region points log for ' + Chat.escapeHTML(leagues[leagueid].name) + '<br /><br />';
	...
	lvl: function (target, room, user) {
		return this.parse('/region lvl ' + target);
	},
	leaguehelp: function (target, room, user) {
		return this.parse('/region help');
	},
};
