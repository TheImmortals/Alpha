...
class Hangman extends Rooms.RoomGame {
   ...
   */
  guess(word, user) {
    if (user.userid === this.creator)
      return user.sendTo(
        this.room,
        "You can't guess in your own hangman game."
      );
  ...
  hangingMan() {
    return `<img width="120" height="120" src="//play.pokemonshowdown.com/fx/hangman${
      this.incorrectGuesses === -1 ? 7 : this.incorrectGuesses
    }.png" />`;
  }
  ...
  generateWindow() {
    ...
    const message =
      result === 1
        ? "Too bad! The mon has been hanged."
        : result === 2
    ...
    if (this.hint)
      output += Chat.html`<div>(Hint: ${this.hint})<br><br>[use /hangman guess (answer) to guess]</div>`;
        ...
        output +=
          ...
          this.letterGuesses
            .map(
              g =>
                `<strong${
                  g[1] === "1" ? "" : ' style="color: #DBA"'
                }>${Chat.escapeHTML(g[0])}</strong>`
            )
            .join(", ");
      }
      if (result === 2) {
  ...
  display(user, broadcast = false) {
    if (broadcast) {
      this.room.add(
        `|uhtml|hangman${this.gameNumber}|${this.generateWindow()}`
      );
    } else {
      user.sendTo(
        this.room,
  ...
  update() {
    this.room.add(
      `|uhtmlchange|hangman${this.gameNumber}|${this.generateWindow()}`
    );
    ...
    this.room.add(
      `|uhtmlchange|hangman${this.gameNumber}|<div class="infobox">(The game of hangman was ended.)</div>`
    );
  ...
  finish() {
    this.room.add(
      `|uhtmlchange|hangman${this.gameNumber}|<div class="infobox">(The game of hangman has ended &ndash; scroll down to see the results)</div>`
    );
  ...
  hangman: {
    ...
    new: function(target, room, user) {
      ...
      if (word.split(" ").some(w => w.length > 20))
        return this.errorReply(
          "Each word in the phrase must be less than 20 characters."
        );
      if (!/[a-zA-Z]/.test(word))
      ...
      return this.privateModAction(
        `(A game of hangman was started by ${user.name}.)`
      );
    },
    createhelp: [
    ...
    guess: function(target, room, user) {
      ...
      if (!room.game || room.game.gameid !== "hangman")
        return this.errorReply(
          "There is no game of hangman running in this room."
        );
      if (!this.canTalk()) return;
    ...
    stop: "end",
      ...
      return this.privateModAction(
        `(The game of hangman was ended by ${user.name}.)`
      );
    },
    endhelp: [
