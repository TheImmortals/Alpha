...
class Poll {
  ...
  generateVotes() {
    let output = `<div class="infobox"><p style="margin: 2px 0 5px 0"><span style="border:1px solid #F90569;color:#F90569;border-radius:4px;padding:0 3px"><i class="fa fa-bar-chart"></i> Vote the poll</span> <strong style="font-size:11pt">${this.getQuestionMarkup()}</strong></p>`;
    this.options.forEach((option, number) => {
      output += `<div style="margin-top: 5px"><button class="button" style="text-align: left" value="/poll vote ${number}" name="send" title="Vote for ${number}. ${Chat.escapeHTML(
        option.name
      )}">${number}. <strong>${this.getOptionMarkup(
        option
      )}</strong></button></div>`;
    });
  ...
  generateResults(ended = false, option = 0) {
    let icon = `<span style="border:1px solid #${
      ended ? "777;color:#555" : "F90569;color:#F90569"
    };border-radius:4px;padding:0 3px"><i class="fa fa-bar-chart"></i> ${
      ended ? "Poll ended" : "Voted"
    }</span>`;
    ...
    let i = iter.next();
    let c = 0;
    let colors = ["#F90569", "#F90569", "#F90569"];
    while (!i.done) {
      let percentage = Math.round(
        (i.value[1].votes * 100) / (this.totalVotes || 1)
      );
      output += `<div style="margin-top: 3px">${i.value[0]}. <strong>${
        i.value[0] === option ? "<em>" : ""
      }${this.getOptionMarkup(i.value[1])}${
        i.value[0] === option ? "</em>" : ""
      }</strong> <small>(${i.value[1].votes} vote${
        i.value[1].votes === 1 ? "" : "s"
      })</small><br /><span style="font-size:7pt;background:${
        colors[c % 3]
      };padding-right:${percentage *
        3}px"></span><small>&nbsp;${percentage}%</small></div>`;
      i = iter.next();
      c++;
    }
    if (option === 0 && !ended)
      output +=
        "<div><small>(You can't vote after viewing results)</small></div>";
    output += "</div>";
  ...
  update() {
    ...
    for (let i in this.room.users) {
      ...
      if (user.userid in this.voters) {
        user.sendTo(
          this.room,
          `|uhtmlchange|poll${this.pollNumber}|${
            results[this.voters[user.userid]]
          }`
        );
        ...
        user.sendTo(
          this.room,
          `|uhtmlchange|poll${this.pollNumber}|${
            results[this.voterIps[user.latestIp]]
          }`
        );
      }
    }
  ...
  updateTo(user, connection = null) {
    ...
    if (user.userid in this.voters) {
      recipient.sendTo(
        this.room,
        `|uhtmlchange|poll${this.pollNumber}|${this.generateResults(
          false,
          this.voters[user.userid]
        )}`
      );
    } else if (user.latestIp in this.voterIps) {
      recipient.sendTo(
      ...
      recipient.sendTo(
        this.room,
        `|uhtmlchange|poll${this.pollNumber}|${this.generateVotes()}`
      );
    }
  }
  ...
  display() {
    ...
    for (let i in this.room.users) {
      ...
      if (thisUser.userid in this.voters) {
        thisUser.sendTo(
          this.room,
          `|uhtml|poll${this.pollNumber}|${
            results[this.voters[thisUser.userid]]
          }`
        );
        ...
        thisUser.sendTo(
          this.room,
          `|uhtml|poll${this.pollNumber}|${
            results[this.voterIps[thisUser.latestIp]]
          }`
        );
      } else {
  ...
  displayTo(user, connection = null) {
    ...
    if (user.userid in this.voters) {
      recipient.sendTo(
        this.room,
        `|uhtml|poll${this.pollNumber}|${this.generateResults(
          false,
          this.voters[user.userid]
        )}`
      );
      ...
      recipient.sendTo(
        this.room,
        `|uhtml|poll${this.pollNumber}|${this.generateVotes()}`
      );
    }
  }
  ...
  onConnect(user, connection = null) {
    ...
    this.room.send(
      `|uhtmlchange|poll${this.pollNumber}|<div class="infobox">(The poll has ended &ndash; scroll down to see the results)</div>`
    );
...
const commands = {
  poll: {
    ...
    new: function(target, room, user, connection, cmd, message) {
      ...
      if (room.poll)
        return this.errorReply(
          "There is already a poll in progress in this room."
        );
      if (params.length < 3)
        return this.errorReply("Not enough arguments for /poll new.");
      ...
      room.poll = new Poll(
        room,
        { source: params[0], supportHTML: supportHTML },
        options
      );
      room.poll.display();
    ...
    vote: function(target, room, user) {
      ...
      if (!room.poll.options.has(parsed))
    ...
    timer: function(target, room, user) {
      ...
      if (target) {
        ...
        if (isNaN(timeout) || timeout <= 0 || timeout > 0x7fffffff)
        ...
        if (room.poll.timeout) {
          return this.sendReply(
            `The poll timer is on and will end in ${room.poll.timeoutMins} minute(s).`
          );
        } else {
